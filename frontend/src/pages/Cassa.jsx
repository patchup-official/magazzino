        ...
        <>
          <div style={{background:'rgba(245,158,11,0.06)',border:'1px solid rgba(245,158,11,0.2)',borderRadius:10,padding:'14px 20px',marginBottom:20,display:'flex',gap:20,flexWrap:'wrap',alignItems:'center'}}>
            ...
            <div style={{flex:1,minWidth:180}}>
              <div style={{color:'#94a3b8',fontSize:11,marginBottom:4,textTransform:'uppercase',letterSpacing:0.4}}>Accantonato (€)</div>
              <div style={{display:'flex',alignItems:'center',background:'#1e293b',border:'1px solid #334155',borderRadius:7,padding:'6px 10px'}}><span style={{color:'#64748b',marginRight:5,fontSize:13}}>€</span><input type="number" min="0" step="0.01" placeholder="0,00" value={accantonato} onChange={e=>setAccantonato(e.target.value)} style={{flex:1,background:'transparent',border:'none',color:'#f1f5f9',fontSize:14,outline:'none'}} /></div>
              <div style={{color:'#475569',fontSize:10,marginTop:3}}>Denaro tenuto in negozio come riserva</div>
            </div>
            <div style={{flex:1,minWidth:180}}>
              <div style={{color:'#94a3b8',fontSize:11,marginBottom:4,textTransform:'uppercase',letterSpacing:0.4}}>Investimento Art. 36 (€)</div>
              ...
              <div style={{color:'#475569',fontSize:10,marginTop:3}}>Totale pagato ai privati per acquisto dispositivi</div>
            </div>
          </div>
          ...
          <SezioneReport emoji="📋" titolo="Note di credito">
            <RigaReport label="Totale note di credito / resi periodo" valore={r.note_credito_tot} colore="#f87171" bold />
          </SezioneReport>
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',zIndex:1000,display:'flex',alignItems:'flex-start',justifyContent:'center',overflowY:'auto',padding:'40px 16px'}}
      ...
      <div style={{background:'#0a0f1e',border:'1px solid #1e293b',borderRadius:16,width:'100%',maxWidth:580,padding:'28px 32px',position:'relative'}}>
        ...
        <Sez title="Incassi" emoji="💳">
          ...
          {nv(s.bonifico)>0&&<R label="Bonifico" val={nv(s.bonifico)} sub />}
          {nv(s.assegni)>0&&<R label="Assegni" val={nv(s.assegni)} sub />}
          {nv(s.compass)>0&&<R label="Compass" val={nv(s.compass)} sub />}
          {nv(s.stripe)>0&&<R label="Stripe" val={nv(s.stripe)} sub />}
          {nv(s.enwon_pay)>0&&<R label="Enwon Pay" val={nv(s.enwon_pay)} sub />}
                  ...
                  <div key={i}
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:20}}>
                      <div style={{textAlign:'right'}}>
                        <div style={{color:'#f1f5f9',fontWeight:700,fontSize:15}}>{fmtE(totV)}</div>
                        <div style={{color:'#64748b',fontSize:11}}>vendite</div>
                      </div>
                      <div style={{textAlign:'right'}}>
                        <div style={{color:Math.abs(diff)<0.01?'#22c55e':'#f87171',fontWeight:700,fontSize:15}}>{fmtE(totI)}</div>
                        <div style={{color:'#64748b',fontSize:11}}>incassato</div>
                      </div>
