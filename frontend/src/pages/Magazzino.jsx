  ...
  const steps = [
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>
          {BRANDS.map(b=>(
            <OptionCard key={b} label={b} selected={f.brand===b} onClick={()=>upd({brand:b})}
              icon={{'Apple':'🍎','Samsung':'📲','Google':'🔍','Xiaomi':'⚡','OnePlus':'🔴','Huawei':'🌐'}[b]}
            />
          ))}
        </div>
      )
    ...
    },
    {
      ...
      content:(
        <div>
          {[
            {val:'A',label:'A — Ottima',icon:'⭐',desc:'Come nuovo, nessun segno di usura'},
            {val:'B',label:'B — Buona', icon:'👍',desc:'Leggeri segni di usura, funziona perfettamente'},
            {val:'C',label:'C — Discreta',icon:'👌',desc:'Graffi evidenti o piccoli difetti ma funzionante'},
          ].map(c=>(
...
function WizardIntervento({ api, device, fornitori, onDone, onClose, editing }) {
  ...
  const steps = [
    ...
    },
    {
      ...
      heading:'👨‍🔧 Chi ha eseguito l\'intervento?',
      subtitle:'Specifica se è stato fatto internamente o da un fornitore esterno.',
      content:(
        <div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:16}}>
            <OptionCard icon="🏠" label="Interno" desc="Eseguito dal nostro tecnico" selected={f.eseguito_da==='interno'} onClick={()=>upd({eseguito_da:'interno'})}/>
...
function WizardRicambio({ api, fornitori, onDone, onClose, editing }) {
  ...
  const CATS = [{val:'batteria',icon:'🔋'},{val:'schermo',icon:'📱'},{val:'scocca',icon:'🔧'},{val:'altoparlante',icon:'🔊'},{val:'fotocamera',icon:'📷'},{val:'connettore',icon:'🔌'},{val:'altro',icon:'📦'}]
            ...
            : <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:10}}>
                {fornitori.map(f=>(
                  <div key={f.id} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:10,padding:'12px 14px'}}>
                    ...
                    {f.telefono&&<div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginTop:2}}>📞 {f.telefono}</div>}
                    ...
                    {interventi.map(i=>(
                      <div key={i.id} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:10,padding:'12px 15px',display:'flex',alignItems:'flex-start',gap:12}}>
                        <div style={{flex:1}}>
                          ...
                          <div style={{fontSize:10.5,color:'rgba(255,255,255,0.25)'}}>{i.data?new Date(i.data).toLocaleDateString('it-IT'):''}{i.fornitore_nome?` · ${i.fornitore_nome}`:''}</div>
                        </div>
      ...
      {alerts.length>0&&(
        <div style={{background:'rgba(245,158,11,0.08)',border:'1px solid rgba(245,158,11,0.2)',borderRadius:10,padding:'12px 16px',marginBottom:16,display:'flex',alignItems:'center',gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:12,color:'rgba(255,255,255,0.45)'}}>{alerts.map(a=>`${a.nome} (${a.qty} pz.)`).join(' · ')}</div>
          </div>
        </div>
      )}
