...
export default function Riparazioni({ api, showToast, autoAction, onAutoActionDone }) {
  ...
  const add = async () => {
    ...
    setModal(false)
    setForm({ cliente:'', tel:'', brand:'Apple', modello:'', problema:'', priorita:'normale', costo:0, data_stimata:'', note:'', ora_inizio:'', durata_minuti:60 })
    showToast('Ã¢ÂÂ Riparazione creata')
  }
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(290px,1fr))', gap:14 }}>
        {filtered.length === 0 ? (
          <div style={{ gridColumn:'1/-1', textAlign:'center', padding:'60px 0', color:'#475569' }}>
            <div style={{ fontSize:32, marginBottom:12, opacity:0.4 }}>Ã¢ÂÂ</div>
          ...
          <div key={r.id} style={{ background:'linear-gradient(135deg, rgba(255,255,255,0.03), rgba(255,255,255,0.01))', border:'1px solid rgba(255,255,255,0.07)', borderRadius:14, padding:18 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:11, color:'#475569', marginBottom:12 }}>
              ...
              <span style={{ fontFamily:'monospace', fontWeight:600 }}>Ã¢ÂÂ¬{r.costo}</span>
            </div>
            <div style={{ display:'flex', gap:8 }}>
              {r.stato==='aperta' && (
                <button onClick={()=>complete(r.id)} style={{ flex:1, background:'rgba(22,163,74,0.15)', border:'1px solid rgba(22,163,74,0.25)', color:'#4ade80', borderRadius:8, padding:'7px 0', fontSize:12, fontWeight:500, cursor:'pointer', fontFamily:'Inter,sans-serif' }}>Ã¢ÂÂ Completata</button>
              )}
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', backdropFilter:'blur(6px)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          <div style={{ background:'#0d1529', border:'1px solid rgba(255,255,255,0.1)', borderRadius:16, padding:24, width:'100%', maxWidth:500, maxHeight:'85vh', overflowY:'auto' }}>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:20 }}>
              ...
              <button onClick={()=>setModal(false)} style={{ background:'none', border:'none', color:'#475569', fontSize:18, cursor:'pointer' }}>Ã¢ÂÂ</button>
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:13 }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                <label style={{ fontSize:11.5, color:'#64748b' }}>PrioritÃÂ </label>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                <label style={{ fontSize:11.5, color:'#64748b' }}>Costo Ã¢ÂÂ¬</label>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
              <div>
                <label style={{ fontSize:11, color:'#64748b', fontWeight:600, textTransform:'uppercase', letterSpacing:'.06em', display:'block', marginBottom:6 }}>Ora inizio</label>
                <input type="time" value={form.ora_inizio} onChange={e=>setForm({...form,ora_inizio:e.target.value})}
                  style={{ background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:8, padding:'8px 10px', color:'#e2e8f0', fontFamily:'Inter,sans-serif', fontSize:13, width:'100%' }} />
              </div>
              <div>
                <label style={{ fontSize:11, color:'#64748b', fontWeight:600, textTransform:'uppercase', letterSpacing:'.06em', display:'block', marginBottom:6 }}>Durata stimata</label>
                <select value={form.durata_minuti} onChange={e=>setForm({...form,durata_minuti:Number(e.target.value)})}
                  style={{ background:'#1e293b', border:'1px solid rgba(255,255,255,0.1)', borderRadius:8, padding:'8px 10px', color:'#e2e8f0', fontFamily:'Inter,sans-serif', fontSize:13, width:'100%' }}>
