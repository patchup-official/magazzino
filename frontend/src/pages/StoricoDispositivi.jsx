...
export default function StoricoDispositivi({showToast}){
  ...
  function ristampaPdf(item){
    ...
    const h='<!DOCTYPE html><html><head><meta charset="utf-8"><title>Doc Vendita '+numOrdine+'</title>'
      ...
      +'table{border-collapse:collapse;width:100%;margin:10px 0}th,td{border:1px solid #ccc;padding:6px 9px}th{background:#f0f0f0}'
      +'.tot{font-weight:bold;background:#e8f5e9}.sec{margin-top:16px;font-weight:bold;border-bottom:2px solid #333;padding-bottom:2px;margin-bottom:6px}'
      ...
      +'<table><tr><th>Nome</th><td>'+(item.venditore_nome||'')+' '+(item.venditore_cognome||'')+'</td><th>Documento</th><td>'+(item.venditore_doc_tipo||'')+' '+(item.venditore_doc_numero||'')+'</td></tr></table>'
      +'<div class="sec">DISPOSITIVO</div>'
      +'<table><tr><th>Tipo/Marca/Modello</th><td>'+(item.dispositivo_tipo||'')+' '+(item.dispositivo_marca||'')+' '+(item.dispositivo_modello||'')+'</td><th>IMEI</th><td>'+(item.dispositivo_imei||'nd')+'</td></tr>'
      +'<tr><th>Colore/Storage</th><td>'+(item.dispositivo_colore||'nd')+' '+(item.dispositivo_storage||'')+'</td><th>Condizione</th><td>'+(item.dispositivo_condizione||'')+'</td></tr></table>'
      ...
      +'<tr><th>Indirizzo</th><td>'+(item.acquirente_indirizzo||'')+' '+(item.acquirente_cap||'')+' '+(item.acquirente_citta||'')+'</td><th>Email</th><td>'+(item.acquirente_email||'nd')+'</td></tr></table>'
