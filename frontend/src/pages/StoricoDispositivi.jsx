// pages/StoricoDispositivi.jsx v1.1
...
function WizardVendita({dispositivo,onSalva,onAnnulla}){
  ...
  function valida(s){
    const e={}
    if(s===0){if(!form.prezzo_vendita||pv<=0)e.prezzo_vendita='Inserisci il prezzo di vendita'}
    if(s===1){
...
<div class="footer">
  ...
  const overlayStyle={position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(8px)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:16}
  const boxStyle={background:'#0a0f1e',border:'1px solid rgba(255,255,255,0.1)',borderRadius:20,width:'100%',maxWidth:560,maxHeight:'90vh',overflowY:'auto',fontFamily:'Inter,sans-serif'}
      ...
      <div style={boxStyle}>
        <div style={{padding:'22px 28px 18px',borderBottom:'1px solid rgba(255,255,255,0.07)'}}>
          ...
          <div style={{fontSize:13,color:'#64748b'}}>
            {dispositivo.dispositivo_marca} {dispositivo.dispositivo_modello}
            {dispositivo.dispositivo_imei?' IMEI '+dispositivo.dispositivo_imei:''}
          </div>
        ...
        <div style={{padding:'22px 28px'}}>
          ...
          {step===0&&(
            <div style={{display:'flex',flexDirection:'column',gap:16}}>
              <div style={{textAlign:'center',padding:'14px 20px',background:'rgba(248,113,113,0.08)',border:'1px solid rgba(248,113,113,0.2)',borderRadius:12}}>
                <div style={{fontSize:11,color:'#64748b',marginBottom:4}}>PREZZO DI ACQUISTO DA PRIVATO</div>
              ...
              {pv>0&&(
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10}}>
                  <div style={{padding:'10px 12px',borderRadius:9,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.1)',textAlign:'center'}}>
                    <div style={{fontSize:10,color:'#64748b',marginBottom:3}}>Costo acquisto</div>
                    <div style={{fontFamily:'monospace',fontWeight:700,fontSize:14,color:'#f87171'}}>{fmtE(pa)}</div>
                  </div>
                  <div style={{padding:'10px 12px',borderRadius:9,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.1)',textAlign:'center'}}>
                    <div style={{fontSize:10,color:'#64748b',marginBottom:3}}>Margine reale</div>
                    <div style={{fontFamily:'monospace',fontWeight:700,fontSize:14,color:'#34d399'}}>{fmtE(margine)}</div>
                  </div>
                  <div style={{padding:'10px 12px',borderRadius:9,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.1)',textAlign:'center'}}>
                    <div style={{fontSize:10,color:'#64748b',marginBottom:3}}>IVA sul margine</div>
                    <div style={{fontFamily:'monospace',fontWeight:700,fontSize:14,color:'#fbbf24'}}>{fmtE(ivaReale)}</div>
                  </div>
                </div>
              )}
          ...
          {step===1&&(
            <div style={{display:'flex',flexDirection:'column',gap:14}}>
                  ...
                  {[['privato','Privato'],['azienda','Azienda']].map(([v,l])=>(
                    <button key={v} onClick={()=>set('acquirente_tipo',v)}
                      style={{flex:1,padding:'9px',borderRadius:9,cursor:'pointer',fontWeight:600,fontSize:13,
                        ...
                        color:form.acquirente_tipo===v?'#fff':'#94a3b8'}}>
                      {l}
                    </button>
                  ))}
                </div>
              ...
              {form.acquirente_tipo==='azienda'?(
                <>
                  <Campo label="Ragione Sociale" required error={errors.acquirente_ragione_sociale}>
                    <input style={inp} value={form.acquirente_ragione_sociale} onChange={e=>{set('acquirente_ragione_sociale',e.target.value);clr('acquirente_ragione_sociale')}} placeholder="Acme Srl"/>
                  </Campo>
                  <Campo label="P.IVA" required error={errors.acquirente_piva}>
                    <input style={inp} value={form.acquirente_piva} onChange={e=>{set('acquirente_piva',e.target.value);clr('acquirente_piva')}} placeholder="IT12345678901"/>
                  </Campo>
                </>
              ):(
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                  <Campo label="Nome" required error={errors.acquirente_nome}>
                    <input style={inp} value={form.acquirente_nome} onChange={e=>{set('acquirente_nome',e.target.value);clr('acquirente_nome')}} placeholder="Mario"/>
                  </Campo>
                  <Campo label="Cognome" required error={errors.acquirente_cognome}>
                    <input style={inp} value={form.acquirente_cognome} onChange={e=>{set('acquirente_cognome',e.target.value);clr('acquirente_cognome')}} placeholder="Rossi"/>
                  </Campo>
                  <Campo label="Codice Fiscale" required error={errors.acquirente_cf}>
                    <input style={inp} value={form.acquirente_cf} onChange={e=>{set('acquirente_cf',e.target.value.toUpperCase());clr('acquirente_cf')}} placeholder="RSSMRA80A01H501Z" maxLength={16}/>
                  </Campo>
                  <Campo label="Telefono">
                    <input style={inp} value={form.acquirente_telefono} onChange={e=>set('acquirente_telefono',e.target.value)} placeholder="+39 333..."/>
                  </Campo>
                </div>
              )}
              <Campo label="Email (per fattura)" required error={errors.acquirente_email}>
                <input type="email" style={inp} value={form.acquirente_email} onChange={e=>{set('acquirente_email',e.target.value);clr('acquirente_email')}} placeholder="mario.rossi@email.com"/>
              </Campo>
              <Campo label="Indirizzo" required error={errors.acquirente_indirizzo}>
                <input style={inp} value={form.acquirente_indirizzo} onChange={e=>{set('acquirente_indirizzo',e.target.value);clr('acquirente_indirizzo')}} placeholder="Via Roma 1"/>
              </Campo>
              <div style={{display:'grid',gridTemplateColumns:'1fr 2fr 1fr',gap:10}}>
                <Campo label="CAP">
                  <input style={inp} value={form.acquirente_cap} onChange={e=>set('acquirente_cap',e.target.value)} placeholder="10100" maxLength={5}/>
                </Campo>
                <Campo label="Citta" required error={errors.acquirente_citta}>
                  <input style={inp} value={form.acquirente_citta} onChange={e=>{set('acquirente_citta',e.target.value);clr('acquirente_citta')}} placeholder="Torino"/>
                </Campo>
                <Campo label="Prov.">
                ...
                {CONDIZIONI.map(c=>(
                  <button key={c.v} onClick={()=>set('dispositivo_condizione',c.v)}
                    style={{flex:1,padding:'8px 4px',borderRadius:8,cursor:'pointer',fontSize:12,fontWeight:600,
                      ...
                      color:form.dispositivo_condizione===c.v?'#fff':'#94a3b8'}}>
                    {c.l}
                  </button>
                ))}
              </div>
...
export default function StoricoDispositivi({showToast}){
  ...
  function ristampaPdf(item){
    ...
    const html='<!DOCTYPE html><html><head><meta charset="utf-8"><title>Doc Vendita '+numOrdine+'</title>'
      +'<style>body{font-family:Arial,sans-serif;font-size:12px;margin:30px;color:#222}'
      +'h1{font-size:18px;margin-bottom:4px}'
      +'.disclaimer{background:#fff3cd;border:2px solid #f59e0b;border-radius:6px;padding:12px 16px;margin:16px 0;font-weight:bold;font-size:13px;color:#92400e;text-align:center}'
      +'table{border-collapse:collapse;width:100%;margin:12px 0}'
      +'th,td{border:1px solid #ccc;padding:7px 10px;text-align:left}'
      +'th{background:#f0f0f0}.totale{font-weight:bold;background:#e8f5e9}'
      +'.footer{margin-top:30px;font-size:10px;color:#888;border-top:1px solid #ccc;padding-top:10px}'
      +'@media print{body{margin:10px}}</style></head><body>'
      +'<h1>Documento di Vendita</h1>'
      +'<div style="font-size:13px;color:#555;margin-bottom:8px">N. Ordine: <strong>'+numOrdine+'</strong> &nbsp;|&nbsp; Data: <strong>'+item.data_vendita+'</strong></div>'
      +'<div class="disclaimer">⚠️ ATTENZIONE: QUESTO DOCUMENTO NON VALE AI FINI FISCALI ⚠️<br>'
      +'Il presente documento è un riepilogo interno della transazione commerciale.<br>'
      +'La fattura fiscale verrà emessa separatamente secondo le disposizioni di legge.</div>'
      +'<div style="margin-top:18px;font-weight:bold;font-size:13px;border-bottom:2px solid #333;padding-bottom:3px;margin-bottom:8px">VENDITORE ORIGINALE (Privato)</div>'
      +'<table><tr><th>Nome</th><td>'+(item.venditore_nome||'')+' '+(item.venditore_cognome||'')+'</td><th>Documento</th><td>'+(item.venditore_doc_tipo||'')+' '+(item.venditore_doc_numero||'')+'</td></tr>'
      +'<tr><th>Telefono</th><td colspan="3">'+(item.venditore_telefono||'—')+'</td></tr></table>'
      +'<div style="margin-top:18px;font-weight:bold;font-size:13px;border-bottom:2px solid #333;padding-bottom:3px;margin-bottom:8px">DISPOSITIVO VENDUTO</div>'
      +'<table>'
      +'<tr><th>Tipo</th><td>'+(item.dispositivo_tipo||'')+'</td><th>Marca / Modello</th><td>'+(item.dispositivo_marca||'')+' '+(item.dispositivo_modello||'')+'</td></tr>'
      +'<tr><th>IMEI / Seriale</th><td>'+(item.dispositivo_imei||'—')+'</td><th>Colore / Storage</th><td>'+(item.dispositivo_colore||'—')+' '+(item.dispositivo_storage||'')+'</td></tr>'
      +'<tr><th>Condizione</th><td>'+(item.dispositivo_condizione||'')+'</td><th>Note</th><td>'+(item.dispositivo_note||'—')+'</td></tr></table>'
      +'<div style="margin-top:18px;font-weight:bold;font-size:13px;border-bottom:2px solid #333;padding-bottom:3px;margin-bottom:8px">ACQUIRENTE</div>'
      +'<table>'
      +'<tr><th>Nome / Ragione Sociale</th><td>'+(nom||'—')+'</td><th>CF / P.IVA</th><td>'+(item.acquirente_cf||item.acquirente_piva||'—')+'</td></tr>'
      +'<tr><th>Indirizzo</th><td>'+(item.acquirente_indirizzo||'')+', '+(item.acquirente_cap||'')+' '+(item.acquirente_citta||'')+' '+(item.acquirente_provincia?'('+item.acquirente_provincia+')':'')+'</td><th>Email</th><td>'+(item.acquirente_email||'—')+'</td></tr>'
      +'<tr><th>Telefono</th><td colspan="3">'+(item.acquirente_telefono||'—')+'</td></tr></table>'
      +'<div style="margin-top:18px;font-weight:bold;font-size:13px;border-bottom:2px solid #333;padding-bottom:3px;margin-bottom:8px">RIEPILOGO ECONOMICO (Art. 36 D.P.R. 633/1972)</div>'
      +'<table>'
      +'<tr><th>Prezzo acquisto da privato</th><td style="text-align:right">€ '+pa.toFixed(2)+'</td></tr>'
      +'<tr><th>Prezzo di vendita</th><td style="text-align:right">€ '+pv.toFixed(2)+'</td></tr>'
      +'<tr><th>Margine di utile (base imponibile)</th><td style="text-align:right">€ '+margine.toFixed(2)+'</td></tr>'
      +'<tr class="totale"><th>IVA dovuta su margine (22%)</th><td style="text-align:right">€ '+ivaReale.toFixed(2)+'</td></tr>'
      +'</table>'
      +(item.operatore?'<p style="font-size:11px;color:#555">Operatore: '+item.operatore+'</p>':'')
      +(item.note_vendita?'<p style="font-size:11px;color:#555">Note: '+item.note_vendita+'</p>':'')
      +'<div class="footer">Documento generato da Enown/Magazzino il '+new Date().toLocaleString('it-IT')+' — N. Ordine '+numOrdine+'<br>'
      +'<strong>QUESTO DOCUMENTO NON SOSTITUISCE LA FATTURA FISCALE</strong> e non ha valore ai fini tributari.<br>'
      +(item.acquirente_email?'La fattura verrà emessa e inviata a '+item.acquirente_email+'.':'')
      +'</div></body></html>';
    const w=window.open('','_blank');
    if(w){w.document.write(html);w.document.close();setTimeout(()=>w.print(),500);}
  }
    ...
    <div style={{maxWidth:1000,margin:'0 auto',fontFamily:'Inter,sans-serif'}}>
      ...
      {(tab==='storico'||tab==='export')&&(
        <div style={{display:'flex',gap:10,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
          ...
          <button onClick={carica} style={{...btnSec,padding:'7px 14px',fontSize:13}}>Aggiorna</button>
        </div>
      )}
      ...
      {tab==='export'&&(
        <div style={{marginBottom:16,padding:18,borderRadius:12,background:'rgba(16,185,129,0.06)',border:'1px solid rgba(16,185,129,0.2)'}}>
          ...
          <div style={{fontSize:13,color:'#94a3b8',marginBottom:14,lineHeight:1.6}}>CSV completo: venditore, dispositivo, acquirente, prezzi, margine, IVA, fattura. Per carico/scarico e Art.36.</div>
          <button onClick={esportaCsv} style={{...btnPrimary,background:'linear-gradient(135deg,#15803d,#16a34a)'}}>Scarica CSV ({lista.length} record)</button>
        </div>
      )}
      ...
      {loading?(
        ...
        <div style={{textAlign:'center',color:'#475569',padding:60,fontSize:14}}>
          ...
          <br/>
          <button onClick={()=>{setEditItem(null);setShowAcquisto(true)}} style={{...btnPrimary,marginTop:14,fontSize:13}}>+ Registra primo acquisto</button>
        </div>
      ):(
