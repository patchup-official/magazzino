...
export default function StoricoDispositivi({showToast}){
  ...
  function ristampaPdf(item){
    ...
    const h='<!DOCTYPE html><html><head><meta charset="utf-8"><title>Doc Vendita '+numOrdine+'</title>'
      +'<style>body{font-family:Arial,sans-serif;font-size:12px;margin:30px}'
      +'.dis{background:#fff3cd;border:2px solid #f59e0b;padding:12px;margin:16px 0;font-weight:bold;text-align:center;color:#92400e}'
      +'table{border-collapse:collapse;width:100%;margin:10px 0}'
      +'th,td{border:1px solid #ccc;padding:6px 9px}th{background:#f0f0f0}'
      +'.tot{font-weight:bold;background:#e8f5e9}'
      +'.sec{margin-top:16px;font-weight:bold;border-bottom:2px solid #333;padding-bottom:2px;margin-bottom:6px}'
      ...
      +'<h2 style="font-size:17px">Documento di Vendita</h2>'
      +'<p style="color:#555">N. Ordine: <b>'+numOrdine+'</b> | Data: <b>'+item.data_vendita+'</b></p>'
      +'<div class="dis">ATTENZIONE: QUESTO DOCUMENTO NON VALE AI FINI FISCALI<br>'
      +'Riepilogo interno — la fattura fiscale verra emessa separatamente.</div>'
      +'<div class="sec">VENDITORE ORIGINALE</div>'
      +'<table><tr><th>Nome</th><td>'+(item.venditore_nome||'')+' '+(item.venditore_cognome||'')+'</td>'
      +'<th>Documento</th><td>'+(item.venditore_doc_tipo||'')+' '+(item.venditore_doc_numero||'')+'</td></tr></table>'
      +'<div class="sec">DISPOSITIVO</div>'
      +'<table><tr><th>Tipo/Marca/Modello</th><td>'+(item.dispositivo_tipo||'')+' '+(item.dispositivo_marca||'')+' '+(item.dispositivo_modello||'')+'</td>'
      +'<th>IMEI/Seriale</th><td>'+(item.dispositivo_imei||'nd')+'</td></tr>'
      +'<tr><th>Colore/Storage</th><td>'+(item.dispositivo_colore||'nd')+' '+(item.dispositivo_storage||'')+'</td>'
      +'<th>Condizione</th><td>'+(item.dispositivo_condizione||'')+'</td></tr></table>'
      +'<div class="sec">ACQUIRENTE</div>'
      +'<table><tr><th>Nome/Ragione Sociale</th><td>'+nom+'</td><th>CF/PIVA</th><td>'+(item.acquirente_cf||item.acquirente_piva||'nd')+'</td></tr>'
      +'<tr><th>Indirizzo</th><td>'+(item.acquirente_indirizzo||'')+' '+(item.acquirente_cap||'')+' '+(item.acquirente_citta||'')+'</td>'
      +'<th>Email</th><td>'+(item.acquirente_email||'nd')+'</td></tr></table>'
      +'<div class="sec">RIEPILOGO ECONOMICO (Art. 36)</div>'
      +'<table><tr><th>Prezzo acquisto da privato</th><td>euro '+pa.toFixed(2)+'</td></tr>'
      +'<tr><th>Prezzo di vendita</th><td>euro '+pv.toFixed(2)+'</td></tr>'
      +'<tr><th>Margine (base imponibile)</th><td>euro '+margine.toFixed(2)+'</td></tr>'
      +'<tr class="tot"><th>IVA su margine (22%)</th><td>euro '+ivaReale.toFixed(2)+'</td></tr></table>'
      +(item.note_vendita?'<p>Note: '+item.note_vendita+'</p>':'')
      +'<div class="ft">Generato da Enown/Magazzino il '+new Date().toLocaleString('it-IT')+'<br>'
      +'QUESTO DOCUMENTO NON SOSTITUISCE LA FATTURA FISCALE.</div></body></html>';
    const w=window.open('','_blank');
