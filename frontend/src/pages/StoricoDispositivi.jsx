// pages/StoricoDispositivi.jsx v1.0
...
function WizardVendita({dispositivo,onSalva,onAnnulla}){
  ...
  const [form,setForm]=useState({data_vendita:oggi(),prezzo_vendita:'',acquirente_tipo:'privato',
    acquirente_nome:'',acquirente_cognome:'',acquirente_email:'',acquirente_telefono:'',
  ...
  const margine=Math.max(0,(+form.prezzo_vendita||0)-dispositivo.prezzo_acquisto)
  ...
  function valida(s){
    const e={}
    if(s===0){if(!form.prezzo_vendita||+form.prezzo_vendita<=0)e.prezzo_vendita='Inserisci prezzo di vendita'}
    if(s===1){
      ...
      if(!form.acquirente_email)e.acquirente_email='Email obbligatoria per invio fattura'
      if(form.acquirente_email&&!/\S+@\S+\.\S+/.test(form.acquirente_email))e.acquirente_email='Email non valida'
      if(!form.acquirente_indirizzo)e.acquirente_indirizzo='Indirizzo obbligatorio per la fattura'
      if(!form.acquirente_citta)e.acquirente_citta='Città obbligatoria'
    }
  ...
  async function conferma(){
    ...
    setSaving(true)
    try{
      const r=await fetch(`${API}/storico-dispositivi/${dispositivo.id}/vendi`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({...form,prezzo_vendita:+form.prezzo_vendita})})
          ...
          <div style={{display:'flex',gap:6,marginTop:14}}>
            {STEPS.map((s,i)=>(
              <div key={i} style={{flex:1,textAlign:'center'}}>
                ...
                <div style={{fontSize:10,color:i<=step?'#60a5fa':'#475569',fontWeight:600}}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
        <div style={{padding:'22px 28px'}}>
          {errors.global&&<div style={{padding:'10px 14px',background:'rgba(239,68,68,0.1)',border:'1px solid rgba(239,68,68,0.25)',borderRadius:8,color:'#f87171',fontSize:13,marginBottom:16}}>{errors.global}</div>}

          {step===0&&<div style={{display:'flex',flexDirection:'column',gap:16}}>
            <div style={{...card('rgba(251,191,36,0.2)'),textAlign:'center'}}>
              <div style={{fontSize:11,color:'#64748b',marginBottom:4}}>PREZZO ACQUISTO</div>
              <div style={{fontSize:22,fontWeight:700,fontFamily:'monospace',color:'#f87171'}}>{fmtE(dispositivo.prezzo_acquisto)}</div>
            </div>
            <Campo label="Prezzo di vendita" required error={errors.prezzo_vendita}>
              <div style={{position:'relative'}}>
                ...
                <input type="number" autoFocus value={form.prezzo_vendita} onChange={e=>{set('prezzo_vendita',e.target.value);clr('prezzo_vendita')}} style={{...inp,paddingLeft:32,fontSize:22,fontFamily:'monospace',fontWeight:700}} step="0.01" min="0" placeholder="0.00"/>
              </div>
            </Campo>
            ...
            <Campo label="Data vendita" required><input type="date" value={form.data_vendita} onChange={e=>set('data_vendita',e.target.value)} style={inp}/></Campo>
          </div>}
          ...
          {step===1&&<div style={{display:'flex',flexDirection:'column',gap:14}}>
            <div>
              ...
              <div style={{display:'flex',gap:8}}>
                {[['privato','👤 Privato'],['azienda','🏢 Azienda']].map(([v,l])=>(
                  <button key={v} onClick={()=>set('acquirente_tipo',v)} style={{flex:1,padding:'9px',borderRadius:9,cursor:'pointer',fontWeight:600,fontSize:13,
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 2fr 1fr',gap:10}}>
              ...
              <Campo label="Città" required error={errors.acquirente_citta}><input style={inp} value={form.acquirente_citta} onChange={e=>{set('acquirente_citta',e.target.value);clr('acquirente_citta')}} placeholder="Torino"/></Campo>
          ...
          {step===2&&<div style={{display:'flex',flexDirection:'column',gap:14}}>
            <div style={{padding:'12px 16px',borderRadius:10,background:'rgba(59,130,246,0.08)',border:'1px solid rgba(59,130,246,0.2)',fontSize:13,color:'#94a3b8',lineHeight:1.6}}>
              📧 Ricordati di inviare la fattura via email a <b style={{color:'#60a5fa'}}>{form.acquirente_email}</b>
            </div>
            <Campo label="Numero fattura" required error={errors.fattura_numero}><input style={inp} value={form.fattura_numero} onChange={e=>{set('fattura_numero',e.target.value);clr('fattura_numero')}} placeholder="2026/001" autoFocus/></Campo>
            ...
            <Campo label="Note"><textarea style={{...inp,resize:'vertical',minHeight:70}} value={form.note_vendita} onChange={e=>set('note_vendita',e.target.value)} placeholder="Note aggiuntive..."/></Campo>
          </div>}

          {step===3&&<div style={{display:'flex',flexDirection:'column',gap:12}}>
            <div style={{fontSize:15,fontWeight:700,color:'#f1f5f9',marginBottom:8}}>✅ Riepilogo vendita</div>
            <div style={card('rgba(59,130,246,0.2)')}>
            ...
            <div style={card('rgba(16,185,129,0.2)')}>
              <div style={{fontSize:11,color:'#34d399',fontWeight:700,marginBottom:8,textTransform:'uppercase'}}>💶 Art.36 — Calcolo IVA su margine</div>
              {[['Prezzo acquisto (da privato)',fmtE(dispositivo.prezzo_acquisto),'#f87171'],['Prezzo vendita',fmtE(+form.prezzo_vendita),'#60a5fa'],['Margine reale',fmtE(margine),'#34d399'],['IVA da versare (22% su margine)',fmtE(ivaReale),'#fbbf24']].map(([n,v,c])=>(
            ...
            <div style={card('rgba(168,85,247,0.2)')}>
              ...
              <div style={{fontSize:12,color:'#64748b',marginTop:2}}>{form.acquirente_email} · {form.acquirente_citta} ({form.acquirente_provincia})</div>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:20,paddingTop:16,borderTop:'1px solid rgba(255,255,255,0.07)'}}>
            ...
            {step<3?<button onClick={avanti} style={btnPrimary}>Avanti →</button>
              :<button onClick={conferma} disabled={saving} style={{...btnPrimary,background:saving?'#1e40af':'linear-gradient(135deg,#15803d,#16a34a)'}}>{saving?'Salvataggio...':'✅ Conferma vendita'}</button>}
          </div>
        </div>
      </div>
    </div>
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(8px)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
      <div style={{background:'#0a0f1e',border:'1px solid rgba(255,255,255,0.1)',borderRadius:20,width:'100%',maxWidth:600,maxHeight:'90vh',overflowY:'auto',fontFamily:'Inter,sans-serif'}}>
        <div style={{padding:'22px 28px 16px',borderBottom:'1px solid rgba(255,255,255,0.07)'}}>
          <div style={{fontSize:16,fontWeight:700,color:'#f1f5f9'}}>{editItem?'✏️ Modifica acquisto':'📥 Registra acquisto da privato'}</div>
        ...
        <div style={{padding:'22px 28px',display:'flex',flexDirection:'column',gap:18}}>
          ...
          <div>
            ...
            <div style={{marginTop:12}}>
              ...
              <div style={{display:'flex',gap:8}}>{CONDIZIONI.map(c=>(<button key={c.v} onClick={()=>set('dispositivo_condizione',c.v)} style={{flex:1,padding:'8px 4px',borderRadius:8,cursor:'pointer',fontSize:12,fontWeight:600,background:form.dispositivo_condizione===c.v?'#2563eb':'rgba(255,255,255,0.05)',border:`1px solid ${form.dispositivo_condizione===c.v?'#3b82f6':'rgba(255,255,255,0.1)'}`,color:form.dispositivo_condizione===c.v?'#fff':'#94a3b8'}}>{c.l}</button>))}</div>
            </div>
          ...
          <div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              <Campo label="Prezzo pagato al privato" required>
                <div style={{position:'relative'}}><span style={{position:'absolute',left:14,top:'50%',transform:'translateY(-50%)',color:'#64748b'}}>€</span><input type="number" style={{...inp,paddingLeft:28,fontSize:18,fontFamily:'monospace',fontWeight:700}} value={form.prezzo_acquisto} onChange={e=>set('prezzo_acquisto',e.target.value)} step="0.01" min="0" placeholder="0.00"/></div>
              </Campo>
          ...
          <div style={{display:'flex',gap:10,justifyContent:'flex-end',paddingTop:8,borderTop:'1px solid rgba(255,255,255,0.07)'}}>
            <button onClick={onAnnulla} style={btnSec}>✕ Annulla</button>
            <button onClick={salva} disabled={saving} style={btnPrimary}>{saving?'Salvataggio...':'✅ Registra acquisto'}</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return(
    <div style={{maxWidth:1000,margin:'0 auto',fontFamily:'Inter,sans-serif'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:10}}>
        ...
        <button onClick={()=>{setEditItem(null);setShowAcquisto(true)}} style={btnPrimary}>➕ Nuovo acquisto</button>
      </div>
      ...
      <div style={{display:'flex',gap:0,borderBottom:'1px solid rgba(255,255,255,0.07)',marginBottom:20}}>
        {[{k:'magazzino',l:'📦 In magazzino'},{k:'storico',l:'📋 Storico completo'},{k:'export',l:'📊 Export fiscale'}].map(({k,l})=>(
          <button key={k} onClick={()=>setTab(k)} style={{padding:'9px 18px',border:'none',cursor:'pointer',fontSize:13,fontWeight:600,background:tab===k?'rgba(255,255,255,0.07)':'transparent',borderBottom:tab===k?'2px solid #3b82f6':'2px solid transparent',color:tab===k?'#e2e8f0':'#64748b'}}>{l}</button>
        ))}
      </div>
      ...
      {(tab==='storico'||tab==='export')&&<div style={{display:'flex',gap:10,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
        ...
        <button onClick={carica} style={{...btnSec,padding:'7px 14px',fontSize:13}}>🔄</button>
      </div>}
      ...
      {tab==='export'&&<div style={{marginBottom:16,padding:18,borderRadius:12,background:'rgba(16,185,129,0.06)',border:'1px solid rgba(16,185,129,0.2)'}}>
        ...
        <div style={{fontSize:13,color:'#94a3b8',marginBottom:14,lineHeight:1.6}}>CSV completo con tutti i dati: venditore, dispositivo, acquirente, prezzi, margine, IVA, fattura.<br/>Utile per carico/scarico e dichiarazione Art.36.</div>
        <button onClick={esportaCsv} style={{...btnPrimary,background:'linear-gradient(135deg,#15803d,#16a34a)'}}>📥 Scarica CSV completo ({lista.length} record)</button>
      </div>}
      ...
      lista.length===0?<div style={{textAlign:'center',color:'#475569',padding:60,fontSize:14}}>
        ...
        <br/><button onClick={()=>{setEditItem(null);setShowAcquisto(true)}} style={{...btnPrimary,marginTop:14,fontSize:13}}>➕ Registra primo acquisto</button>
      </div>:
          ...
          <div key={item.id} style={{padding:'14px 18px',borderRadius:11,background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
            <div style={{flex:1,minWidth:200}}>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4,flexWrap:'wrap'}}>
                ...
                <span style={{padding:'2px 8px',borderRadius:20,fontSize:11,fontWeight:700,background:item.stato==='venduto'?'#34d39922':'#fbbf2422',color:item.stato==='venduto'?'#34d399':'#fbbf24',border:`1px solid ${item.stato==='venduto'?'#34d39944':'#fbbf2444'}`}}>{item.stato==='venduto'?'Venduto':'In magazzino'}</span>
              ...
              <div style={{fontSize:12,color:'#64748b',display:'flex',gap:12,flexWrap:'wrap'}}>
                ...
                {item.stato==='venduto'&&<span>→ {item.acquirente_nome} {item.acquirente_cognome||item.acquirente_ragione_sociale} ({item.data_vendita})</span>}
              </div>
            </div>
            <div style={{display:'flex',gap:12,alignItems:'center',flexWrap:'wrap'}}>
              ...
              {item.prezzo_vendita>0&&<><div style={{textAlign:'right'}}><div style={{fontSize:10,color:'#64748b'}}>Vendita</div><div style={{fontFamily:'monospace',fontWeight:700,color:'#34d399',fontSize:14}}>{fmtE(item.prezzo_vendita)}</div></div>
            ...
            <div style={{display:'flex',gap:6}}>
              ...
              <button onClick={()=>{setEditItem(item);setShowAcquisto(true)}} style={{padding:'6px 10px',borderRadius:7,cursor:'pointer',background:'rgba(96,165,250,0.15)',border:'1px solid rgba(96,165,250,0.3)',color:'#60a5fa',fontSize:12}}>✏️</button>
              <button onClick={()=>elimina(item.id)} style={{padding:'6px 10px',borderRadius:7,cursor:'pointer',background:'rgba(248,113,113,0.1)',border:'1px solid rgba(248,113,113,0.2)',color:'#f87171',fontSize:12}}>🗑️</button>
            </div>
          </div>
        ))}
      </div>}
