...
const inp  = { width: '100%', padding: '9px 12px', borderRadius: 8, border: '1px solid #334155', background: '#0f172a', color: '#f1f5f9', fontSize: 13, boxSizing: 'border-box' }
...
export default function AdminDashboard() {
  ...
  // Form nuovo store — solo nome e città, codice automatico
  ...
  async function createStore(e) {
    e.preventDefault()
    const r = await authFetch('/admin/stores', { method: 'POST', body: JSON.stringify(newStore) })
    const data = await r.json()
    if (r.ok) {
      showToast('✅ Store creato! Codice: ' + data.code)
      setNewStore({ name: '', city: '' })
      fetchStores()
    } else {
  ...
  async function createAdmin(e) {
    e.preventDefault()
    const r = await authFetch('/admin/users', { method: 'POST', body: JSON.stringify(newUser) })
    const data = await r.json()
    if (r.ok) {
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#f1f5f9', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ background: '#1e293b', borderBottom: '1px solid #334155', padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button onClick={logout} style={{ ...btn('#ef4444'), padding: '6px 14px', fontSize: 12 }}>Esci</button>
        </div>
      </div>
        ...
        {tab === 'stores' && (
          <>
            <div style={card}>
              ...
              <form onSubmit={createStore} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 10, alignItems: 'end' }}>
                <div>
                  <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>NOME STORE</label>
                  <input style={inp} placeholder="PatchUP Palermo" value={newStore.name}
                    onChange={e => setNewStore(s => ({ ...s, name: e.target.value }))} required />
                </div>
                <div>
                  <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>CITTÀ</label>
                  <input style={inp} placeholder="Palermo" value={newStore.city}
                    onChange={e => setNewStore(s => ({ ...s, city: e.target.value }))} required />
                </div>
                <button type="submit" style={btn()}>Crea store</button>
              </form>
              <p style={{ color: '#475569', fontSize: 11, margin: '10px 0 0' }}>
                Il codice store viene generato automaticamente
              </p>
            </div>
            ...
            <div style={card}>
              ...
              {loading ? <p style={{ color: '#64748b' }}>Caricamento...</p> : stores.map(s => (
                <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #1e293b' }}>
                  <div>
                    <span style={{ fontWeight: 600 }}>{s.name}</span>
                    <span style={{ color: '#64748b', fontSize: 13, marginLeft: 10 }}>{s.city}</span>
                    <span style={{ ...badge('#6366f1'), marginLeft: 8 }}>{s.code}</span>
                    <span style={{ color: '#64748b', fontSize: 12, marginLeft: 8 }}>{s.operators_count} operatori</span>
                    {!s.active && <span style={{ ...badge('#ef4444'), marginLeft: 6 }}>inattivo</span>}
                  </div>
        ...
        {tab === 'users' && (
          <>
            <div style={card}>
              ...
              <form onSubmit={createAdmin} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr auto', gap: 10, alignItems: 'end' }}>
                <div>
                  ...
                  <input style={inp} placeholder="Mario Rossi" value={newUser.name}
                ...
                <div>
                  ...
                  <input style={inp} placeholder="mario.rossi" value={newUser.username}
                ...
                <div>
                  ...
                  <input style={inp} type="password" placeholder="••••••••" value={newUser.password}
                ...
                <div>
                  ...
                  <select style={inp} value={newUser.store_id}
                    onChange={e => setNewUser(u => ({ ...u, store_id: e.target.value }))} required>
                    <option value="">Seleziona store</option>
                    {stores.map(s => <option key={s.id} value={s.id}>{s.name} ({s.code})</option>)}
                  </select>
                </div>
                <button type="submit" style={{ ...btn(), whiteSpace: 'nowrap' }}>Crea admin</button>
              </form>
            </div>
            ...
            <div style={card}>
              ...
              {users.map(u => (
                <div key={u.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #1e293b' }}>
                  <div>
                    ...
                    {!u.active && <span style={{ ...badge('#ef4444'), marginLeft: 6 }}>inattivo</span>}
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
