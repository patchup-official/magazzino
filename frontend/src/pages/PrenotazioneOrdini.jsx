...
setLoading(true);
try {
const r = await fetch(`${BASE}/clienti?search=${encodeURIComponent(val)}`);
const d = await r.json();
setClienti(Array.isArray(d) ? d : (d?.data ? d.data : []));
} catch(e) {}
setLoading(false);
};
...
if (!newC.nome) return;
try {
const r = await fetch(`${BASE}/clienti`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(newC) });
const d = await r.json();
setSelected(d?.data || d); setShowNew(false);
} catch(e) {}
};
...
<div style={{ fontSize:13, fontWeight:500, color:'#e2e8f0' }}>{fullName(c)}</div>
<div style={{ fontSize:11, color:'#475569' }}>{[c.telefono, c.email].filter(Boolean).join(' - ')}</div>
</div>
...
<div style={{ flex:1, minWidth:0 }}>
<div style={{ fontSize:13, fontWeight:500, color:'#e2e8f0' }}>{d.brand} {d.modello}{d.colore_storage?` - ${d.colore_storage}`:''}</div>
<div style={{ fontSize:11, color:'#475569' }}>{d.imei?`IMEI: ${d.imei}`:'Nessun IMEI'}</div>
</div>
...
{isSel && <span style={{ color:'#22c55e', flexShrink:0 }}>ok</span>}
</button>
);
})}
</div>
...
<CardWrap icon="box" title="Ricambio" subtitle="Intervento senza ricambi dal catalogo"
...
{!loading && results.length===0 && q && <div style={{ padding:'16px', textAlign:'center', color:'#475569', fontSize:13 }}>Nessun risultato</div>}
{results.map(p => {
...
<span style={{ fontSize:10, padding:'1px 6px', borderRadius:20, background:p.instock==='Y'?'#0f3320':'#3b0f0f', color:p.instock==='Y'?'#4ade80':'#f87171' }}>{p.instock==='Y'?'Disp.':'No'}</span>
...
{cartSent ? <span style={{ fontSize:12, color:'#22c55e', marginLeft:'auto' }}>Nel carrello Foneday</span>
: <button onClick={addCart} disabled={cartSending} style={{ ...S.btnPrimary, marginLeft:'auto', fontSize:12, padding:'6px 14px', opacity:cartSending?.6:1 }}>{cartSending?'...':'Aggiungi al carrello Foneday'}</button>}
</div>
...
{ricambio && <Row label="Ricambio Foneday"><div><div>{ricambio.title}</div><div style={{ fontSize:11, color:'#475569', marginTop:2 }}>SKU: {ricambio.sku} - €{ricambio.price?.toFixed(2)} x {ricambio.qty}</div>{ricambio.cartSent && <span style={{ fontSize:10, padding:'1px 6px', borderRadius:20, background:'#0f3320', color:'#4ade80', display:'inline-block', marginTop:3 }}>Nel carrello Foneday</span>}</div></Row>}
...
const r = await fetch(`${BASE}/prenotazioni-ordini?${p}`);
const d = await r.json();
...
colore_variante: dispositivo.colore_storage||null,
...
setViewMode('lista'); setStep(0);
setData({ cliente:null, dispositivo:null, assistenza:null, ricambio:null, caparra:null });
...
{STATI.map(s => (
<div key={s.key} onClick={()=>setFiltro(filtro===s.key?'':s.key)} style={{ background:filtro===s.key?s.bg:'rgba(255,255,255,0.03)', border:`1px solid ${filtro===s.key?s.text:'rgba(255,255,255,0.08)'}`, borderRadius:10, padding:14, cursor:'pointer', textAlign:'center' }}>
...
: lista.length===0 ? (
<div style={{ padding:'2.5rem', textAlign:'center', color:'rgba(255,255,255,0.25)' }}>
<div style={{ fontSize:28, marginBottom:8 }}>Nessuna prenotazione</div>
<button onClick={()=>setViewMode('wizard')} style={{ marginTop:12, ...S.btnGhost, color:'#60a5fa', borderColor:'rgba(96,165,250,0.3)' }}>Crea la prima</button>
</div>
) : lista.map(p => {
...
<select value={p.stato} onChange={e=>updateStato(p.id,e.target.value)} style={{ fontSize:11, padding:'3px 8px', borderRadius:20, border:'none', background:s.bg, color:s.text, cursor:'pointer', fontFamily:'inherit', fontWeight:600 }}>
...
{p.cliente_citta && <span style={{ fontSize:11, color:'#94a3b8' }}>{p.cliente_citta}</span>}
