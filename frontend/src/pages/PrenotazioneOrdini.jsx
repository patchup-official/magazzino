...
const QUALITY_COLORS = { 'Service Pack':{ bg:'#1e3a5f', text:'#60a5fa' },'Pulled':{ bg:'#1a1060', text:'#a78bfa' },'Refurbished':{ bg:'#3b1a06', text:'#fb923c' },'OEM-Equivalent':{ bg:'#0f3320', text:'#4ade80' },'FDX':{ bg:'#1e1060', text:'#818cf8' } };
function qColor(q) { for (const [k,v] of Object.entries(QUALITY_COLORS)) if (q && q.includes(k)) return v; return { bg:'rgba(255,255,255,0.06)', text:'#94a3b8' }; }
const S = {
...
return (<div style={{ marginBottom:24 }}><div style={{ display:'flex', alignItems:'center' }}>
{STEP_LABELS.map((l,i) => (<div key={i} style={{ display:'flex', alignItems:'center', flex:i<STEP_LABELS.length-1?1:'none' }}>
<div style={{ width:28, height:28, borderRadius:'50%', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:500, zIndex:1, background:i<=current?'#22c55e':'rgba(255,255,255,0.04)', border:i<=current?'2px solid #22c55e':'1px solid rgba(255,255,255,0.12)', color:i<=current?'white':'rgba(255,255,255,0.3)' }}>{i<current?'v':i+1}</div>
{i<STEP_LABELS.length-1 && <div style={{ flex:1, height:1.5, background:i<current?'#22c55e':'rgba(255,255,255,0.08)' }}/>}
</div>))}
</div><div style={{ display:'flex', marginTop:6 }}>
{STEP_LABELS.map((l,i) => <div key={i} style={{ flex:i<STEP_LABELS.length-1?1:'none', fontSize:10, color:i===current?'#e2e8f0':i<current?'#22c55e':'rgba(255,255,255,0.25)', fontWeight:i===current?600:400 }}>{l}</div>)}
</div></div>);
}
...
const [q,setQ]=useState('');const [clienti,setClienti]=useState([]);const [loading,setLoading]=useState(false);const [selected,setSelected]=useState(null);const [showNew,setShowNew]=useState(false);const [newC,setNewC]=useState({ nome:'',cognome:'',telefono:'',email:'' });
const search=async(val)=>{ if(!val||val.length<2){setClienti([]);return;} setLoading(true); try{ const r=await fetch(`${BASE}/clienti?search=${encodeURIComponent(val)}`);const d=await r.json();setClienti(Array.isArray(d)?d:(d?.data||[])); }catch(e){} setLoading(false); };
const saveNew=async()=>{ if(!newC.nome)return; try{ const r=await fetch(`${BASE}/clienti`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(newC)});const d=await r.json();setSelected(d?.data||d);setShowNew(false); }catch(e){} };
...
return (<CardWrap icon="👤" title="Seleziona cliente" subtitle="Cerca nel registro o aggiungi nuovo"
...
{(clienti.length>0||loading)&&(<div style={{ border:'1px solid rgba(255,255,255,0.08)', borderRadius:8, overflow:'hidden', maxHeight:240, overflowY:'auto' }}>
{loading&&<div style={{ padding:'12px 16px', fontSize:13, color:'#475569' }}>Ricerca...</div>}
{clienti.map(c=>(<div key={c.id} onClick={()=>setSelected(c)} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', borderBottom:'1px solid rgba(255,255,255,0.05)', cursor:'pointer', background:selected?.id===c.id?'rgba(34,197,94,0.1)':'transparent' }}>
...
{selected?.id===c.id&&<span style={{ color:'#22c55e' }}>✓</span>}
</div>))}
...
const [q,setQ]=useState('');const [dispositivi,setDispositivi]=useState([]);const [loading,setLoading]=useState(false);const [selected,setSelected]=useState(null);const [showNew,setShowNew]=useState(false);const [newD,setNewD]=useState({ brand:'',modello:'',colore_storage:'',imei:'' });
...
const loadDev=async()=>{ setLoading(true); try{ const r=await fetch(`${BASE}/devices?cliente_id=${cliente.id}`);const d=await r.json();setDispositivi(Array.isArray(d)?d:[]); }catch(e){ setDispositivi([]); } setLoading(false); };
const saveNew=async()=>{ if(!newD.brand||!newD.modello)return; try{ const r=await fetch(`${BASE}/devices`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...newD,cliente_id:cliente?.id})});const d=await r.json();setSelected(d);setShowNew(false);loadDev(); }catch(e){} };
const filtered=dispositivi.filter(d=>!q||[d.brand,d.modello,d.imei,d.colore_storage].join(' ').toLowerCase().includes(q.toLowerCase()));
const fullName=c=>[c.nome,c.cognome].filter(Boolean).join(' ');
return (<CardWrap icon="📱" title="Dispositivo del cliente" subtitle={`Cliente: ${fullName(cliente)}`}
...
{filtered.map(d=>(<div key={d.id} onClick={()=>setSelected(d)} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', borderBottom:'1px solid rgba(255,255,255,0.05)', cursor:'pointer', background:selected?.id===d.id?'rgba(34,197,94,0.1)':'transparent' }}>
...
{selected?.id===d.id&&<span style={{ color:'#22c55e' }}>✓</span>}
</div>))}
...
<div><label style={S.lbl}>Modello *</label><input value={newD.modello} onChange={e=>setNewD(p=>({...p,modello:e.target.value}))} placeholder="es. Reno 13f" style={S.inp}/></div>
...
return (<CardWrap icon="🔧" title="Tipo di assistenza" subtitle={`${dispositivo.brand} ${dispositivo.modello}`}
footer={<><button onClick={onBack} style={S.btnSecondary}>Indietro</button><button style={{ ...S.btnPrimary, opacity:selected?1:.35 }} disabled={!selected} onClick={()=>onNext(selected)}>{selected?.fonedayQ?'Cerca su Foneday →':'Avanti →'}</button></>}>
...
{cat.items.map(item=>{ const isSel=selected?.label===item.label; return (
...
{isSel&&<span style={{ color:'#22c55e', flexShrink:0 }}>✓</span>}
</button>); })}
</div></div>))}
</CardWrap>);
}
...
const search=useCallback(async(val)=>{ if(!val||val.trim().length<2)return; setLoading(true);setError(''); try{ const r=await fetch(`${BASE}/foneday/search?q=${encodeURIComponent(val)}&instock=${instockOnly}`);const d=await r.json();if(!r.ok)throw new Error(d.error);setResults(d.products||[]);if(d.products?.length===0)setShowManuale(true); }catch(e){ setError(e.message);setShowManuale(true); } setLoading(false); },[instockOnly,BASE]);
useEffect(()=>{ if(autoQ)search(autoQ); },[]);

const addCart=async()=>{ if(!selected)return; setCartSending(true); try{ const r=await fetch(`${BASE}/foneday/cart/add`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({articles:[{sku:selected.sku,quantity:qty,note:null}]})});if(!r.ok)throw new Error((await r.json()).error);setCartSent(true); }catch(e){ setError(e.message); } setCartSending(false); };

const canProceed = selected || (showManuale && manuale.trim().length > 0);
const nextPayload = selected ? { ...selected, qty, cartSent } : showManuale && manuale ? { title:manuale, sku:null, price:null, qty:1, cartSent:false, manuale:true } : null;

if(!assistenza.fonedayQ) return (
<CardWrap icon="📦" title="Ricambio" subtitle="Intervento senza ricambi dal catalogo"
...
<p style={{ fontSize:13, color:'#475569', textAlign:'center', padding:'20px 0' }}>Servizio senza ricambio fisico.</p>
</CardWrap>);
return (<CardWrap icon="search" title="Ricambio da Foneday" subtitle={assistenza.label + ' - ' + dispositivo.brand + ' ' + dispositivo.modello}
footer={<>
<button onClick={onBack} style={S.btnSecondary}>Indietro</button>
<div style={{ display:'flex', gap:8 }}>
<button onClick={()=>{ setShowManuale(true);setSelected(null); }} style={{ ...S.btnGhost, borderColor:'rgba(251,191,36,0.3)', color:'#fbbf24' }}>✏️ Inserisci manuale</button>
<button style={{ ...S.btnPrimary, opacity:canProceed?1:.35 }} disabled={!canProceed} onClick={()=>onNext(nextPayload)}>Avanti</button>
</div></>}>
...
<input type="checkbox" checked={instockOnly} onChange={e=>{setInstockOnly(e.target.checked);search(q);}}/> Solo disponibili
</label>
{showManuale&&(<div style={{ marginBottom:10, padding:'12px 14px', background:'rgba(251,191,36,0.06)', border:'1px solid rgba(251,191,36,0.2)', borderRadius:8 }}>
<label style={{ ...S.lbl, color:'#fbbf24' }}>Ricambio manuale</label>
<input value={manuale} onChange={e=>{setManuale(e.target.value);setSelected(null);}} placeholder="es. Display LCD Oppo Reno 13f Nero Originale" style={{ ...S.inp, borderColor:'rgba(251,191,36,0.3)' }}/>
<div style={{ fontSize:11, color:'#92400e', marginTop:5 }}>Foneday non ha trovato risultati - inserisci manualmente e vai avanti.</div>
</div>)}
...
{results.map(p=>{ const qc=qColor(p.quality);const isSel=selected?.sku===p.sku; return (
<div key={p.sku} onClick={()=>{ setSelected(p);setCartSent(false);setManuale(''); }} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', borderBottom:'1px solid rgba(255,255,255,0.05)', cursor:'pointer', background:isSel?'rgba(34,197,94,0.08)':'transparent', opacity:p.instock==='N'?.5:1 }}>
<div style={{ flex:1, minWidth:0 }}>
<div style={{ fontSize:12, fontWeight:500, color:'#e2e8f0', marginBottom:3, lineHeight:1.3 }}>{p.title}</div>
<div style={{ display:'flex', gap:5 }}><span style={{ fontSize:10, padding:'1px 6px', borderRadius:20, background:p.instock==='Y'?'#0f3320':'#3b0f0f', color:p.instock==='Y'?'#4ade80':'#f87171' }}>{p.instock==='Y'?'Disp.':'No'}</span><span style={{ fontSize:10, padding:'1px 6px', borderRadius:20, background:qc.bg, color:qc.text }}>{p.quality}</span></div>
</div>
<div style={{ textAlign:'right', flexShrink:0 }}>
<div style={{ fontSize:13, fontWeight:600, color:'#e2e8f0' }}>€{p.price?.toFixed(2)}</div>
{isSel&&<span style={{ fontSize:11, color:'#22c55e' }}>ok</span>}
</div></div>); })}
</div>
{selected&&(<div style={{ marginTop:12, padding:'12px 14px', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8 }}>
...
<span style={{ fontSize:13, fontWeight:600, color:'#22c55e' }}>€{(selected.price*qty).toFixed(2)}</span>
{cartSent?<span style={{ fontSize:12, color:'#22c55e', marginLeft:'auto' }}>Nel carrello Foneday</span>:<button onClick={addCart} disabled={cartSending} style={{ ...S.btnPrimary, marginLeft:'auto', fontSize:12, padding:'6px 14px', opacity:cartSending?.6:1 }}>{cartSending?'...':'Aggiungi al carrello Foneday'}</button>}
</div>
{cartSent&&<a href="https://foneday.shop/it/cart" target="_blank" rel="noreferrer" style={{ display:'block', marginTop:6, fontSize:11, color:'#6366f1' }}>Apri carrello</a>}
</div>)}
</CardWrap>);
}

function Step5({ onNext, onBack }) {
const [hasCap,setHasCap]=useState(false);const [importo,setImporto]=useState('');const [totale,setTotale]=useState('');const [metodo,setMetodo]=useState('Contanti');const [note,setNote]=useState('');
const saldo=hasCap&&importo&&totale?(parseFloat(totale)-parseFloat(importo)).toFixed(2):null;
return (<CardWrap icon="cash" title="Caparra" subtitle="Importo anticipato dal cliente (opzionale)"
...
{hasCap&&(<><div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:12 }}>
<div><label style={S.lbl}>Importo caparra</label><input type="number" value={importo} min={0} step={0.01} onChange={e=>setImporto(e.target.value)} placeholder="0.00" style={S.inp}/></div>
<div><label style={S.lbl}>Totale preventivo</label><input type="number" value={totale} min={0} step={0.01} onChange={e=>setTotale(e.target.value)} placeholder="0.00" style={S.inp}/></div>
</div>
<div style={{ display:'flex', gap:8, marginBottom:12 }}>{['Contanti','Bonifico'].map(m=><button key={m} onClick={()=>setMetodo(m)} style={{ fontSize:12, padding:'5px 16px', borderRadius:20, border:'1px solid', cursor:'pointer', fontFamily:'inherit', background:metodo===m?'rgba(34,197,94,0.15)':'transparent', borderColor:metodo===m?'#22c55e':'rgba(255,255,255,0.12)', color:metodo===m?'#4ade80':'#94a3b8' }}>{m}</button>)}</div>
...
return (<CardWrap icon="check" title="Riepilogo prenotazione" subtitle="Controlla e conferma"
...
{ricambio&&<Row label="Ricambio">{ricambio.manuale&&<span style={{ fontSize:10, padding:'1px 6px', borderRadius:20, background:'rgba(251,191,36,0.15)', color:'#fbbf24', marginRight:6 }}>Manuale</span>}{ricambio.title}{ricambio.price&&<span style={{ color:'#22c55e', marginLeft:8 }}>€{ricambio.price.toFixed(2)} x{ricambio.qty}</span>}{ricambio.cartSent&&<span style={{ fontSize:10, padding:'1px 6px', borderRadius:20, background:'#0f3320', color:'#4ade80', marginLeft:6 }}>Foneday</span>}</Row>}
...
const BASE=api||API;const [step,setStep]=useState(0);const [data,setData]=useState({ cliente:null,dispositivo:null,assistenza:null,ricambio:null,caparra:null });const [saving,setSaving]=useState(false);const [lista,setLista]=useState([]);const [loadingLista,setLoadingLista]=useState(false);const [viewMode,setViewMode]=useState('lista');const [filtro,setFiltro]=useState('');const [searchQ,setSearchQ]=useState('');
...
const loadLista=useCallback(async()=>{ setLoadingLista(true); try{ const p=new URLSearchParams();if(filtro)p.set('stato',filtro);if(searchQ)p.set('search',searchQ);const r=await fetch(BASE+'/prenotazioni-ordini?'+p);const d=await r.json();setLista(Array.isArray(d)?d:[]); }catch(e){ setLista([]); } setLoadingLista(false); },[filtro,searchQ,BASE]);
useEffect(()=>{ if(viewMode==='lista')loadLista(); },[viewMode,loadLista]);
const save=async()=>{ setSaving(true); try{ const { cliente,dispositivo,assistenza,ricambio,caparra }=data;const body={ cliente_id:cliente.id||null,cliente_nome:[cliente.nome,cliente.cognome].filter(Boolean).join(' '),cliente_telefono:cliente.telefono||null,cliente_email:cliente.email||null,brand:dispositivo.brand,modello:dispositivo.modello,colore_variante:dispositivo.colore_storage||null,tipo_riparazione:assistenza.label,ricambio:ricambio?.title||null,in_store:true,stato:'da_ordinare',data_inserimento:new Date().toISOString().split('T')[0],caparra_attiva:caparra?.attiva||false,caparra_importo:caparra?.importo?parseFloat(caparra.importo):null,caparra_totale:caparra?.totale?parseFloat(caparra.totale):null,caparra_metodo:caparra?.metodo||null,caparra_note:caparra?.note||null };const r=await fetch(BASE+'/prenotazioni-ordini',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});if(!r.ok)throw new Error(await r.text());showToast&&showToast('Prenotazione salvata');setViewMode('lista');setStep(0);setData({ cliente:null,dispositivo:null,assistenza:null,ricambio:null,caparra:null }); }catch(e){ showToast?showToast('Errore: '+e.message,'error'):alert(e.message); } setSaving(false); };
const delPren=async(id)=>{ if(!confirm('Eliminare?'))return;await fetch(BASE+'/prenotazioni-ordini/'+id,{method:'DELETE'});showToast&&showToast('Eliminata');loadLista(); };
const updateStato=async(id,stato)=>{ const extra=stato==='ordinato'?{data_ordine:new Date().toISOString().split('T')[0]}:stato==='arrivato'?{data_arrivo:new Date().toISOString().split('T')[0]}:{};await fetch(BASE+'/prenotazioni-ordini/'+id,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({stato,...extra})});loadLista(); };
if(viewMode==='wizard') return (<div style={{ maxWidth:660, margin:'0 auto' }}>
<div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}><h1 style={{ fontSize:18, fontWeight:700, color:'#e2e8f0', margin:0 }}>Nuova prenotazione</h1><button onClick={()=>setViewMode('lista')} style={{ ...S.btnSecondary, fontSize:12 }}>Annulla</button></div>
...
<input value={searchQ} onChange={e=>setSearchQ(e.target.value)} placeholder="Cerca cliente, brand, modello..." style={{ ...S.inp, marginBottom:14 }}/>
...
<div key={p.id} style={{ display:'grid', gridTemplateColumns:'auto 1fr auto', gap:12, alignItems:'center', padding:'11px 16px', borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
<select value={p.stato} onChange={e=>updateStato(p.id,e.target.value)} style={{ fontSize:11, padding:'3px 8px', borderRadius:20, border:'none', background:s.bg, color:s.text, cursor:'pointer', fontFamily:'inherit', fontWeight:600 }}>{STATI.map(x=><option key={x.key} value={x.key}>{x.label}</option>)}</select>
<div style={{ minWidth:0 }}>
<div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:3 }}>
<span style={{ fontWeight:600, fontSize:13, color:'#e2e8f0' }}>{p.cliente_nome}</span>
{!!p.in_store&&<span style={{ fontSize:10, padding:'1px 6px', borderRadius:20, background:'#0f3320', color:'#4ade80', fontWeight:500 }}>in store</span>}
</div>
<div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
<span style={{ fontSize:12, color:'#60a5fa', fontWeight:500 }}>{p.brand} {p.modello}{p.colore_variante?' '+p.colore_variante:''}</span>
{p.ricambio&&<span style={{ fontSize:11, padding:'1px 7px', borderRadius:20, background:'rgba(99,102,241,0.15)', color:'#818cf8' }}>{p.ricambio}</span>}
{p.tipo_riparazione&&!p.ricambio&&<span style={{ fontSize:11, color:'#475569' }}>{p.tipo_riparazione}</span>}
</div>
</div>
<div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:4, flexShrink:0 }}>
{!!p.caparra_attiva&&p.caparra_totale&&<span style={{ fontSize:13, fontWeight:700, color:'#22c55e' }}>€ {parseFloat(p.caparra_totale).toFixed(2)}</span>}
{p.data_inserimento&&<span style={{ fontSize:10, color:'#334155' }}>{p.data_inserimento}</span>}
<button onClick={()=>delPren(p.id)} style={{ background:'none', border:'none', cursor:'pointer', color:'#475569', fontSize:10, padding:0, fontFamily:'inherit' }}>elimina</button>
</div>
</div>); })}
</div>
</div>);
