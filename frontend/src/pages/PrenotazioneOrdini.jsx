...
const loadDev=async()=>{setLoad(true);try{
// GET /devices?cliente_id= risponde {data:[...]} o array diretto
const r=await fetch(BASE+'/devices?cliente_id='+cliente.id);const d=await r.json();
setDevs(Array.isArray(d)?d:Array.isArray(d?.data)?d.data:[]);
}catch(e){setDevs([]);}setLoad(false);};
const saveNew=async()=>{if(!nD.brand||!nD.modello)return;try{
const body={brand:nD.brand,modello:nD.modello,colore:nD.colore,imei:nD.imei||null,cliente_id:cliente?.id};
const r=await fetch(BASE+'/devices',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
const d=await r.json();
// Normalizza risposta
const dev=d?.data||d;
// Aggiungi colore_storage per compatibilità con il resto del wizard
dev.colore_storage=dev.colore||nD.colore;
setSel(dev);setShowNew(false);loadDev();
}catch(e){}};
const filt=devs.filter(d=>!q||[d.brand,d.modello,d.imei,d.colore,d.colore_storage].join(' ').toLowerCase().includes(q.toLowerCase()));
const fn=c=>[c.nome,c.cognome].filter(Boolean).join(' ');
const devLabel=d=>d.brand+' '+d.modello+(d.colore_storage||d.colore?' - '+(d.colore_storage||d.colore):'');
...
<div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:500,color:'#e2e8f0'}}>{devLabel(d)}</div><div style={{fontSize:11,color:'#475569'}}>{d.imei?'IMEI: '+d.imei:'Nessun IMEI'}</div></div>
...
const subT=dispositivo.brand+' '+dispositivo.modello;
return(<Card icon="🔧" title="Tipo di assistenza" subtitle={subT}
...
{cat.items.map(item=>{const iS=sel?.label===item.label;return(
<button key={item.label} onClick={()=>setSel(item)} style={{display:'flex',alignItems:'center',gap:8,padding:'9px 11px',borderRadius:8,border:'1px solid '+(iS?'#22c55e':'rgba(255,255,255,0.08)'),background:iS?'rgba(34,197,94,0.12)':CB[cat.cat]||'rgba(255,255,255,0.02)',cursor:'pointer',fontFamily:'inherit',fontSize:12.5,color:iS?'#4ade80':'#e2e8f0',textAlign:'left',width:'100%',fontWeight:iS?600:400}}>
...
const search=useCallback(async v=>{if(!v||v.trim().length<2)return;setLoad(true);setErr('');
try{const r=await fetch(BASE+'/foneday/search?q='+encodeURIComponent(v)+'&instock='+instock);const d=await r.json();if(!r.ok)throw new Error(d.error);setRes(d.products||[]);if(!(d.products?.length))setShowM(true);}
catch(e){setErr(e.message);setShowM(true);}setLoad(false);},[instock,BASE]);
useEffect(()=>{if(autoQ)search(autoQ);},[]);
const addCart=async()=>{if(!sel)return;setSending(true);
try{const r=await fetch(BASE+'/foneday/cart/add',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({articles:[{sku:sel.sku,quantity:qty,note:null}]})});if(!r.ok)throw new Error((await r.json()).error);setSent(true);}
catch(e){setErr(e.message);}setSending(false);};
const canProceed=sel||(showM&&manuale.trim().length>0);
const nextVal=sel?{...sel,qty,cartSent:sent}:showM&&manuale?{title:manuale,sku:null,price:null,qty:1,cartSent:false,manuale:true}:null;
if(!assistenza.fonedayQ)return(<Card icon="📦" title="Ricambio" subtitle="Intervento senza ricambi dal catalogo"
footer={<><button onClick={onBack} style={S.btnS}>Indietro</button><button style={S.btnP} onClick={()=>onNext(null)}>Avanti</button></>}>
<p style={{fontSize:13,color:'#475569',textAlign:'center',padding:'20px 0'}}>Servizio senza ricambio fisico.</p></Card>);
return(<Card icon="🔍" title="Ricambio da Foneday" subtitle={assistenza.label+' - '+dispositivo.brand+' '+dispositivo.modello}
footer={<><button onClick={onBack} style={S.btnS}>Indietro</button><div style={{display:'flex',gap:8}}>
<button onClick={()=>{setShowM(true);setSel(null);}} style={{...S.btnG,borderColor:'rgba(251,191,36,0.3)',color:'#fbbf24'}}>✏️ Manuale</button>
<button style={{...S.btnP,opacity:canProceed?1:.35}} disabled={!canProceed} onClick={()=>onNext(nextVal)}>Avanti →</button>
</div></>}>
...
<button onClick={()=>search(q)} disabled={load} style={{...S.btnG,whiteSpace:'nowrap',background:'rgba(99,102,241,0.15)',borderColor:'rgba(99,102,241,0.4)',color:'#818cf8'}}>{load?'...':'Cerca'}</button>
</div>
<label style={{display:'flex',alignItems:'center',gap:6,fontSize:12,color:'#475569',cursor:'pointer',marginBottom:10}}>
<input type="checkbox" checked={instock} onChange={e=>{setInstock(e.target.checked);search(q);}}/> Solo disponibili</label>
{showM&&(<div style={{marginBottom:10,padding:'12px 14px',background:'rgba(251,191,36,0.06)',border:'1px solid rgba(251,191,36,0.2)',borderRadius:8}}>
<label style={{...S.lbl,color:'#fbbf24'}}>Ricambio manuale</label>
<input value={manuale} onChange={e=>{setManuale(e.target.value);setSel(null);}} placeholder="es. Display LCD Oppo Reno 13f Nero Originale" style={{...S.inp,borderColor:'rgba(251,191,36,0.3)'}}/>
<div style={{fontSize:11,color:'#92400e',marginTop:5}}>Inserisci il ricambio manualmente e premi Avanti.</div>
</div>)}
...
{res.map(p=>{const qc=qColor(p.quality);const iS=sel?.sku===p.sku;return(
<div key={p.sku} onClick={()=>{setSel(p);setSent(false);setManuale('');}} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',borderBottom:'1px solid rgba(255,255,255,0.05)',cursor:'pointer',background:iS?'rgba(34,197,94,0.08)':'transparent',opacity:p.instock==='N'?.5:1}}>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:500,color:'#e2e8f0',marginBottom:3,lineHeight:1.3}}>{p.title}</div>
<div style={{display:'flex',gap:5}}><span style={{fontSize:10,padding:'1px 6px',borderRadius:20,background:p.instock==='Y'?'#0f3320':'#3b0f0f',color:p.instock==='Y'?'#4ade80':'#f87171'}}>{p.instock==='Y'?'Disp.':'No'}</span><span style={{fontSize:10,padding:'1px 6px',borderRadius:20,background:qc.bg,color:qc.text}}>{p.quality}</span></div></div>
...
{sel&&(<div style={{marginTop:12,padding:'12px 14px',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:8}}>
<div style={{fontSize:12,fontWeight:500,color:'#e2e8f0',marginBottom:8}}>{sel.title}</div>
...
<span style={{fontSize:13,fontWeight:600,color:'#22c55e'}}>€{(sel.price*qty).toFixed(2)}</span>
{sent?<span style={{fontSize:12,color:'#22c55e',marginLeft:'auto'}}>✓ Nel carrello Foneday</span>:<button onClick={addCart} disabled={sending} style={{...S.btnP,marginLeft:'auto',fontSize:12,padding:'6px 14px',opacity:sending?.6:1}}>{sending?'...':'Aggiungi al carrello Foneday'}</button>}
</div>
{sent&&<a href="https://foneday.shop/it/cart" target="_blank" rel="noreferrer" style={{display:'block',marginTop:6,fontSize:11,color:'#6366f1'}}>Apri carrello →</a>}
</div>)}
</Card>);}
...
return(<Card icon="💵" title="Caparra" subtitle="Importo anticipato dal cliente (opzionale)"
footer={<><button onClick={onBack} style={S.btnS}>Indietro</button><button style={S.btnP} onClick={()=>onNext({attiva:hasCap,importo:imp,totale:tot,metodo:met,note})}>Avanti</button></>}>
<div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
<span style={{fontSize:13,color:'#e2e8f0'}}>Il cliente ha lasciato una caparra?</span>
<div style={{display:'flex',gap:6}}>{['Si','No'].map(v=><button key={v} onClick={()=>setHasCap(v==='Si')} style={{fontSize:12,padding:'5px 16px',borderRadius:20,border:'1px solid',cursor:'pointer',fontFamily:'inherit',background:(v==='Si'&&hasCap)||(v==='No'&&!hasCap)?'rgba(34,197,94,0.15)':'transparent',borderColor:(v==='Si'&&hasCap)||(v==='No'&&!hasCap)?'#22c55e':'rgba(255,255,255,0.12)',color:(v==='Si'&&hasCap)||(v==='No'&&!hasCap)?'#4ade80':'#94a3b8'}}>{v}</button>)}</div>
</div>
{hasCap&&(<><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}}>
<div><label style={S.lbl}>Importo caparra (€)</label><input type="number" value={imp} min={0} step={0.01} onChange={e=>setImp(e.target.value)} placeholder="0.00" style={S.inp}/></div>
<div><label style={S.lbl}>Totale preventivo (€)</label><input type="number" value={tot} min={0} step={0.01} onChange={e=>setTot(e.target.value)} placeholder="0.00" style={S.inp}/></div>
</div>
...
const devLabel=dispositivo?(dispositivo.brand+' '+dispositivo.modello+(dispositivo.colore_storage||dispositivo.colore?' - '+(dispositivo.colore_storage||dispositivo.colore):'')):'—';
return(<Card icon="✅" title="Riepilogo prenotazione" subtitle="Controlla e conferma"
footer={<><button onClick={onBack} style={S.btnS}>Indietro</button><button onClick={onSave} disabled={saving} style={{...S.btnP,opacity:saving?.6:1}}>{saving?'Salvataggio...':'Salva prenotazione'}</button></>}>
<Row label="Cliente">{fn(cliente)}{cliente.telefono?' - '+cliente.telefono:''}</Row>
<Row label="Dispositivo">{devLabel}</Row>
...
cliente_nome:[cliente.nome,cliente.cognome].filter(Boolean).join(' '),
cliente_telefono:cliente.telefono||null,
cliente_email:cliente.email||null,
brand:dispositivo.brand,
modello:dispositivo.modello,
colore_variante:dispositivo.colore_storage||dispositivo.colore||null,
tipo_riparazione:assistenza.label,
ricambio:ricambio?.title||null,
in_store:true,stato:'da_ordinare',
data_inserimento:new Date().toISOString().split('T')[0],
caparra_attiva:caparra?.attiva||false,
caparra_importo:caparra?.importo?parseFloat(caparra.importo):null,
caparra_totale:caparra?.totale?parseFloat(caparra.totale):null,
caparra_metodo:caparra?.metodo||null,
caparra_note:caparra?.note||null,
};
...
if(!r.ok)throw new Error(resp.error||JSON.stringify(resp));
...
{STATI.map(s=>(<div key={s.key} onClick={()=>setFiltro(filtro===s.key?'':s.key)} style={{background:filtro===s.key?s.bg:'rgba(255,255,255,0.03)',border:'1px solid '+(filtro===s.key?s.text:'rgba(255,255,255,0.08)'),borderRadius:10,padding:14,cursor:'pointer',textAlign:'center'}}><div style={{fontSize:22,fontWeight:700,color:filtro===s.key?s.text:'#e2e8f0'}}>{cnt(s.key)}</div><div style={{fontSize:12,color:'#475569',marginTop:3}}>{s.label}</div></div>))}
</div>
<input value={sq} onChange={e=>setSq(e.target.value)} placeholder="Cerca cliente, brand, modello..." style={{...S.inp,marginBottom:14}}/>
...
<select value={p.stato} onChange={e=>updStato(p.id,e.target.value)} style={{fontSize:11,padding:'3px 8px',borderRadius:20,border:'none',background:s.bg,color:s.text,cursor:'pointer',fontFamily:'inherit',fontWeight:600}}>{STATI.map(x=><option key={x.key} value={x.key}>{x.label}</option>)}</select>
...
<button onClick={()=>del(p.id)} style={{background:'none',border:'none',cursor:'pointer',color:'#475569',fontSize:10,padding:0,fontFamily:'inherit'}}>elimina</button>
</div>
</div>);})}
</div>
</div>);}
