...
footer={<><button onClick={onBack} style={S.btnS}>Indietro</button><div style={{display:'flex',gap:8}}><button onClick={()=>{setShowM(true);setSel(null);}} style={{...S.btnG,borderColor:'rgba(34,197,94,0.4)',color:'#22c55e'}}>✏️ Inserisci manuale</button><button style={{...S.btnP,opacity:canP?1:.35}} disabled={!canP} onClick={()=>onNext(nextV)}>Avanti →</button></div></>}>
{err&&!showM&&<div style={{marginBottom:10,padding:'8px 12px',background:'rgba(248,113,113,0.1)',borderRadius:7,fontSize:12,color:'#f87171'}}>{err}</div>}
...
{/* FIX 1: Box manuale con sfondo chiaro e testo grande */}
{showM&&(<div style={{marginBottom:12,padding:'16px',background:'rgba(34,197,94,0.08)',border:'2px solid rgba(34,197,94,0.3)',borderRadius:10}}>
<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
<span style={{fontSize:18}}>✏️</span>
<div>
<div style={{fontSize:13,fontWeight:600,color:'#4ade80'}}>Inserisci ricambio manualmente</div>
<div style={{fontSize:11,color:'#94a3b8',marginTop:1}}>Foneday non ha trovato risultati — scrivi il nome del ricambio e premi Avanti</div>
</div>
</div>
<input value={manuale} onChange={e=>{setManuale(e.target.value);setSel(null);}} placeholder="es. Display LCD Oppo Reno 13f Nero Originale" style={{...S.inp,borderColor:'rgba(34,197,94,0.4)',background:'rgba(34,197,94,0.05)',fontSize:14}} autoFocus/>
</div>)}

<div style={{border:'1px solid rgba(255,255,255,0.08)',borderRadius:8,overflow:'hidden',maxHeight:200,overflowY:'auto'}}>
{load&&<div style={{padding:'16px',textAlign:'center',color:'#475569',fontSize:13}}>Ricerca in corso...</div>}
{!load&&res.length===0&&q&&!showM&&<div style={{padding:'16px',textAlign:'center',color:'#475569',fontSize:13}}>Nessun risultato per "{q}"</div>}
...
return(<Card icon="cash" title="Caparra" subtitle="Importo anticipato dal cliente (opzionale)" footer={<><button onClick={onBack} style={S.btnS}>Indietro</button><button style={S.btnP} onClick={()=>onNext({attiva:hasCap,importo:imp,totale:tot,metodo:met,note})}>Avanti</button></>}>
...
{saldo!==null&&<div style={{display:'flex',justifyContent:'space-between',padding:'10px 14px',background:'rgba(34,197,94,0.08)',border:'1px solid rgba(34,197,94,0.2)',borderRadius:8}}><span style={{fontSize:13,color:'#475569'}}>Saldo residuo</span><span style={{fontSize:14,fontWeight:600,color:'#22c55e'}}>E {saldo}</span></div>}
</>)}
</Card>);}
...
return(<Card icon="check" title="Riepilogo prenotazione" subtitle="Controlla e conferma" footer={<><button onClick={onBack} style={S.btnS}>Indietro</button><button onClick={onSave} disabled={saving} style={{...S.btnP,opacity:saving?.6:1}}>{saving?'Salvataggio...':'Salva prenotazione'}</button></>}>
...
{ricambio&&<Row label="Ricambio">{ricambio.manuale&&<span style={{fontSize:10,padding:'1px 6px',borderRadius:20,background:'rgba(34,197,94,0.15)',color:'#4ade80',marginRight:6}}>Manuale</span>}{ricambio.title}</Row>}
{caparra?.attiva&&<Row label="Caparra">E{caparra.importo} {caparra.metodo}{caparra.totale?' - saldo E'+(parseFloat(caparra.totale)-parseFloat(caparra.importo)).toFixed(2):''}</Row>}
...
const BASE=api||API;
const[step,setStep]=useState(0);const[data,setData]=useState({cliente:null,dispositivo:null,assistenza:null,ricambio:null,caparra:null});const[saving,setSaving]=useState(false);
const[lista,setLista]=useState([]);const[loadL,setLoadL]=useState(false);const[viewMode,setViewMode]=useState('lista');const[filtro,setFiltro]=useState('');const[sq,setSq]=useState('');
...
return(<div>
<div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}><div><h1 style={{fontSize:20,fontWeight:700,color:'#e2e8f0',margin:0}}>Prenotazione Ordini</h1><p style={{fontSize:13,color:'#475569',marginTop:3}}>Gestione ricambi per riparazioni</p></div><button onClick={()=>setViewMode('wizard')} style={S.btnP}>+ Nuova prenotazione</button></div>
<div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:18}}>{STATI.map(s=>(<div key={s.key} onClick={()=>setFiltro(filtro===s.key?'':s.key)} style={{background:filtro===s.key?s.bg:'rgba(255,255,255,0.03)',border:'1px solid '+(filtro===s.key?s.text:'rgba(255,255,255,0.08)'),borderRadius:10,padding:14,cursor:'pointer',textAlign:'center'}}><div style={{fontSize:22,fontWeight:700,color:filtro===s.key?s.text:'#e2e8f0'}}>{cnt(s.key)}</div><div style={{fontSize:12,color:'#475569',marginTop:3}}>{s.label}</div></div>))}</div>
...
<thead><tr>
<th style={th}>Stato</th><th style={th}>Nome</th><th style={th}>Contatto</th><th style={th}>Modello</th><th style={th}>Ricambio</th>
<th style={th}>Preventivo</th><th style={th}>Data</th><th style={{...th,textAlign:'center'}}>Store</th><th style={th}>Fornitore</th><th style={th}>Note</th><th style={th}></th>
</tr></thead>
<tbody>{lista.map(p=>{const s=STATI.find(x=>x.key===p.stato)||STATI[0];
const prevStr=p.caparra_totale?'E '+parseFloat(p.caparra_totale).toFixed(2):p.caparra_importo&&p.caparra_attiva?'E '+parseFloat(p.caparra_importo).toFixed(2)+' cap.':null;
return(<tr key={p.id} style={{transition:'background .1s'}} onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,0.02)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
<td style={{...td,padding:'8px 12px'}}><select value={p.stato} onChange={e=>updStato(p.id,e.target.value)} style={{fontSize:10,padding:'2px 7px',borderRadius:20,border:'none',background:s.bg,color:s.text,cursor:'pointer',fontFamily:'inherit',fontWeight:600}}>{STATI.map(x=><option key={x.key} value={x.key}>{x.label}</option>)}</select></td>
<td style={td}><span style={{fontWeight:600}}>{p.cliente_nome}</span></td>
...
<td style={{...td,fontWeight:700,color:'#22c55e'}}>{prevStr||<span style={{color:'#334155',fontWeight:400}}>—</span>}</td>
...
<td style={{...td,padding:'8px 8px'}}><button onClick={()=>del(p.id)} style={{background:'none',border:'1px solid rgba(248,113,113,0.2)',borderRadius:6,cursor:'pointer',color:'#f87171',fontSize:11,padding:'2px 8px',fontFamily:'inherit'}}>X</button></td>
</tr>);})}</tbody>
</table>)}
</div>
</div>);}
