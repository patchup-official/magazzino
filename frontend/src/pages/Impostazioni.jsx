...
const API_FB='https://magazzino-backend-f7vr.onrender.com'
...
function ProtPage({onBack,showToast,api,initialTab}){
  ...
  useEffect(()=>{fetch((api||API_FB)+'/piani').then(r=>r.ok?r.json():[]).then(d=>setPiani(Array.isArray(d)?d:[])).catch(()=>{});},[api])
  const salvaP=async()=>{if(!form.nome||!form.prezzo)return showToast('Campi obbligatori','error');setSaving(true);try{const body=JSON.stringify({...form,prezzo:+form.prezzo,durata_mesi:+form.durata_mesi});if(editItem){await fetch((api||API_FB)+'/piani/'+editItem.id,{method:'PUT',headers:{'Content-Type':'application/json'},body})}else{await fetch(api+'/piani',{method:'POST',headers:{'Content-Type':'application/json'},body})};const res=await fetch((api||API_FB)+'/piani').then(r=>r.json());setPiani(res);setShowForm(false);setEditItem(null);setForm({nome:'',prezzo:'',durata_mesi:'12',coperture:[]});showToast(editItem?'Piano aggiornato':'Piano creato')}catch{showToast('Errore salvataggio','error')}finally{setSaving(false)}}
  const delP=async(id)=>{if(!confirm('Eliminare questo piano di protezione?'))return;await fetch((api||API_FB)+'/piani/'+id,{method:'DELETE'});setPiani(piani.filter(p=>p.id!==id));showToast('Piano eliminato')}
