    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 14px', borderRadius: 20, background: 'rgba(0,200,150,0.1)', border: '1px solid rgba(0,200,150,0.3)' }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: SUBBYX_TEAL }} />
          <span style={{ color: SUBBYX_TEAL, fontSize: 12, fontWeight: 600 }}>API Connessa</span>
        </div>
      </div>
      <input placeholder="🔍  Cerca dispositivo..." value={search} onChange={e => setSearch(e.target.value)}
        style={{ width: '100%', padding: '11px 16px', borderRadius: 10, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#e2e8f0', fontSize: 14, outline: 'none', marginBottom: 16, boxSizing: 'border-box' }} />
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
        {CATEGORIES.map(c => (
          <button key={c.key} onClick={() => setCategory(c.key)} style={{ padding: '6px 14px', borderRadius: 20, border: '1px solid', borderColor: category === c.key ? SUBBYX_TEAL : 'rgba(255,255,255,0.1)', background: category === c.key ? 'rgba(0,200,150,0.12)' : 'rgba(255,255,255,0.03)', color: category === c.key ? SUBBYX_TEAL : '#64748b', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>{c.icon} {c.label}</button>
        ))}
      </div>
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
          {filtered.map(p => (
            <button key={p.id} onClick={() => onSelect(p)} style={{ padding: 16, borderRadius: 14, background: 'rgba(15,23,42,0.8)', border: '1px solid rgba(255,255,255,0.07)', cursor: 'pointer', textAlign: 'left', position: 'relative' }}>
              {p.condition === 'Eccellente' && <div style={{ position: 'absolute', top: 10, right: 10, padding: '2px 8px', borderRadius: 10, background: 'rgba(99,102,241,0.2)', color: '#a5b4fc', fontSize: 10, fontWeight: 600 }}>PRELOVED</div>}
              <div style={{ fontSize: 36, marginBottom: 10 }}>{p.image}</div>
              <div style={{ fontSize: 11, color: '#64748b', fontWeight: 600 }}>{p.brand}</div>
...
function StepCalcolatore({ product, onProceed, onBack }) {
  const [piano, setPiano] = useState('johnnyx');
  const [mesi, setMesi] = useState(12);
  const rata = piano === 'johnnyx' ? product.johnnyx : teddyxRate(product.teddyx_start, Math.ceil(mesi / 2));
  const totale = piano === 'johnnyx' ? product.johnnyx * mesi : Array.from({ length: mesi }, (_, i) => teddyxRate(product.teddyx_start, i + 1)).reduce((a, b) => a + b, 0);
  const teddyxPreview = Array.from({ length: Math.min(mesi, 6) }, (_, i) => ({ mese: i + 1, rata: teddyxRate(product.teddyx_start, i + 1) }));
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
        <button onClick={() => setPiano('johnnyx')} style={{ padding: 20, borderRadius: 14, textAlign: 'left', cursor: 'pointer', background: piano === 'johnnyx' ? 'rgba(0,200,150,0.08)' : 'rgba(255,255,255,0.03)', border: `2px solid ${piano === 'johnnyx' ? SUBBYX_TEAL : 'rgba(255,255,255,0.08)'}` }}>
        ...
        <button onClick={() => setPiano('teddyx')} style={{ padding: 20, borderRadius: 14, textAlign: 'left', cursor: 'pointer', background: piano === 'teddyx' ? 'rgba(99,102,241,0.08)' : 'rgba(255,255,255,0.03)', border: `2px solid ${piano === 'teddyx' ? '#6366f1' : 'rgba(255,255,255,0.08)'}` }}>
          <div style={{ fontSize: 22, marginBottom: 6 }}>🐻</div>
          <div style={{ fontSize: 16, fontWeight: 800, color: piano === 'teddyx' ? '#a5b4fc' : '#e2e8f0', marginBottom: 4 }}>Teddyx</div>
          <div style={{ fontSize: 12, color: '#64748b', marginBottom: 12, lineHeight: 1.5 }}>Rata che si abbassa progressivamente. Tieni il device finché vuoi.</div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 12, padding: 20, marginBottom: 16 }}>
        ...
        <input type="range" min="1" max="36" value={mesi} onChange={e => setMesi(Number(e.target.value))} style={{ width: '100%', accentColor: SUBBYX_TEAL, cursor: 'pointer' }} />
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#475569', marginTop: 4 }}><span>1 mese</span><span>36 mesi</span></div>
      </div>
      {piano === 'teddyx' && (
      ...
      <div style={{ background: piano === 'johnnyx' ? 'rgba(0,200,150,0.06)' : 'rgba(99,102,241,0.06)', border: `1px solid ${piano === 'johnnyx' ? 'rgba(0,200,150,0.2)' : 'rgba(99,102,241,0.2)'}`, borderRadius: 14, padding: 20, marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}><span style={{ color: '#94a3b8', fontSize: 14 }}>Piano selezionato</span><span style={{ color: '#e2e8f0', fontWeight: 600 }}>{piano === 'johnnyx' ? '⚡ Johnnyx' : '🐻 Teddyx'}</span></div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}><span style={{ color: '#94a3b8', fontSize: 14 }}>Durata</span><span style={{ color: '#e2e8f0', fontWeight: 600 }}>{mesi} mesi</span></div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}><span style={{ color: '#94a3b8', fontSize: 14 }}>Rata {piano === 'teddyx' ? 'media' : ''}</span><span style={{ color: piano === 'johnnyx' ? SUBBYX_TEAL : '#a5b4fc', fontWeight: 700, fontSize: 18 }}>{fmt(rata)}/mese</span></div>
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 10, display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#64748b', fontSize: 13 }}>Totale stimato ({mesi} mesi)</span><span style={{ color: '#e2e8f0', fontWeight: 700 }}>{fmt(totale)}</span></div>
        <div style={{ marginTop: 8, fontSize: 11, color: '#475569' }}>✓ Senza vincoli · ✓ Assistenza inclusa · ✓ Recesso libero</div>
      </div>
      <button onClick={() => onProceed({ piano, mesi, rata, totale })} style={{ width: '100%', padding: '14px', borderRadius: 12, border: 'none', background: `linear-gradient(135deg, ${SUBBYX_TEAL}, #00A878)`, color: '#fff', fontSize: 16, fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 20px rgba(0,200,150,0.3)' }}>
  ...
  if (success) {
    return (
      <div style={{ textAlign: 'center', padding: '32px 0' }}>
        ...
        <h3 style={{ color: '#00C896', fontSize: 22, fontWeight: 800, margin: '0 0 8px' }}>Noleggio attivato!</h3>
        ...
        <div style={{ background: 'rgba(0,200,150,0.08)', border: '1px solid rgba(0,200,150,0.2)', borderRadius: 14, padding: 20, marginBottom: 24, textAlign: 'left' }}>
          {[['ID Ordine', orderId], ['Cliente', `${form.nome} ${form.cognome}`], ['Dispositivo', `${product.brand} ${product.model}`], ['Piano', config.piano === 'johnnyx' ? '⚡ Johnnyx' : '🐻 Teddyx'], ['Rata', `${fmt(config.rata)}/mese`], ['Durata', `${config.mesi} mesi`], ['Stato', '🟢 Confermato — Spedizione entro 2 gg']].map(([k, v]) => (
        ...
        <button onClick={onDone} style={{ padding: '12px 32px', borderRadius: 10, background: '#6366f1', border: 'none', color: '#fff', cursor: 'pointer', fontSize: 14, fontWeight: 600 }}>Nuovo noleggio</button>
      </div>
    );
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', background: 'rgba(0,200,150,0.06)', border: '1px solid rgba(0,200,150,0.15)', borderRadius: 12, marginBottom: 20 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 20, fontWeight: 800, color: '#00C896' }}>{fmt(config.rata)}<span style={{ fontSize: 11, color: '#64748b', fontWeight: 400 }}>/mese</span></div>
          <div style={{ fontSize: 11, color: '#64748b' }}>{config.mesi} mesi · {config.piano === 'johnnyx' ? 'Johnnyx' : 'Teddyx'}</div>
        </div>
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
        <Field label="NOME *" k="nome" placeholder="Mario" half />
        <Field label="COGNOME *" k="cognome" placeholder="Rossi" half />
        <Field label="EMAIL *" k="email" placeholder="mario.rossi@email.it" />
        <Field label="TELEFONO *" k="telefono" placeholder="+39 340 1234567" half />
        <Field label="CODICE FISCALE" k="cf" placeholder="RSSMRA80A01H501Z" half />
        <Field label="INDIRIZZO *" k="indirizzo" placeholder="Via Roma 1" />
        <Field label="CAP *" k="cap" placeholder="20100" half />
        <Field label="CITTÀ *" k="citta" placeholder="Milano" half />
        <Field label="NOTE" k="note" placeholder="Note per la spedizione..." />
      </div>
      <div style={{ fontSize: 11, color: '#475569', marginBottom: 16, padding: '10px 14px', background: 'rgba(255,255,255,0.02)', borderRadius: 8, lineHeight: 1.6 }}>
        Attivando il noleggio il cliente accetta i <span style={{ color: '#00C896' }}>Termini e Condizioni Subbyx</span>. Il contratto sarà gestito da Subbyx S.r.l. Spedizione entro 2 gg lavorativi.
      </div>
      <button onClick={attiva} disabled={loading || !form.nome || !form.cognome || !form.email} style={{ width: '100%', padding: '14px', borderRadius: 12, border: 'none', background: loading || !form.nome ? 'rgba(0,200,150,0.3)' : 'linear-gradient(135deg, #00C896, #00A878)', color: '#fff', fontSize: 16, fontWeight: 700, cursor: loading || !form.nome ? 'not-allowed' : 'pointer' }}>
...
function PannelloImpostazioni({ onSave }) {
  ...
  async function testApi() { setTesting(true); setTestOk(null); await new Promise(r => setTimeout(r, 1500)); setTestOk(apiKey.length > 0); setTesting(false); }
  function save() { setSaved(true); setTimeout(() => { setSaved(false); onSave && onSave(); }, 1500); }
  return (
    <div style={{ maxWidth: 560 }}>
      <h3 style={{ color: '#e2e8f0', marginTop: 0, marginBottom: 6 }}>Impostazioni API Subbyx</h3>
      <p style={{ color: '#64748b', fontSize: 13, marginBottom: 24 }}>Inserisci le credenziali fornite da Subbyx per attivare il plugin. Contatta <span style={{ color: '#00C896' }}>partner@subbyx.com</span></p>
      <div style={{ background: 'rgba(15,23,42,0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 24, marginBottom: 20 }}>
        {[{ label: 'API KEY *', val: apiKey, set: setApiKey, placeholder: 'sbx_live_xxxxxxxxxxxxxxxxxxxx', type: 'password' }, { label: 'PARTNER CODE', val: partnerCode, set: setPartnerCode, placeholder: 'ENWON-001', type: 'text' }].map((f, i) => (
          <div key={i} style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 11, color: '#475569', fontWeight: 600, letterSpacing: 0.5, marginBottom: 6 }}>{f.label}</div>
            <input type={f.type} value={f.val} placeholder={f.placeholder} onChange={e => f.set(e.target.value)} style={{ width: '100%', padding: '10px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#e2e8f0', fontSize: 14, boxSizing: 'border-box', outline: 'none' }} />
          </div>
        ))}
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <button onClick={testApi} disabled={testing || !apiKey} style={{ padding: '9px 18px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: '#94a3b8', cursor: apiKey ? 'pointer' : 'not-allowed', fontSize: 13 }}>
            {testing ? '⏳ Test...' : '🔌 Testa connessione'}
          </button>
          {testOk === true && <span style={{ color: '#00C896', fontSize: 13 }}>✓ Connessione riuscita!</span>}
          {testOk === false && <span style={{ color: '#ef4444', fontSize: 13 }}>✗ API Key non valida</span>}
        </div>
      </div>
      <button onClick={save} style={{ padding: '12px 28px', borderRadius: 10, border: 'none', background: saved ? '#22c55e' : 'linear-gradient(135deg, #00C896, #00A878)', color: '#fff', cursor: 'pointer', fontSize: 14, fontWeight: 700 }}>
...
export default function Noleggio({ api, showToast, onNavigate }) {
  const [tab, setTab] = useState('noleggio');
  ...
  const storico = [
    ...
    { id: 'SUB-294710', cliente: 'Luca Neri', device: 'Galaxy S25 Ultra', piano: 'Johnnyx', rata: 54.90, data: '01/04/2026', stato: 'spedito' },
  ];
  const TABS = [{ key: 'noleggio', label: '📦 Nuovo Noleggio' }, { key: 'storico', label: '📋 Storico' }, { key: 'impostazioni', label: '🔑 Impostazioni API' }];
  return (
    <div style={{ padding: '24px 28px', maxWidth: 1000, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            ...
            <div style={{ padding: '3px 10px', borderRadius: 20, background: 'rgba(0,200,150,0.12)', border: '1px solid rgba(0,200,150,0.3)', fontSize: 11, fontWeight: 700, color: '#00C896' }}>BETA</div>
          </div>
          <p style={{ margin: 0, color: '#64748b', fontSize: 13 }}>Abbonamenti flessibili senza vincoli · Powered by <a href="https://subbyx.com" target="_blank" rel="noreferrer" style={{ color: '#00C896', fontWeight: 700, textDecoration: 'none' }}>Subbyx</a></p>
        </div>
        <div style={{ display: 'flex', gap: 12 }}>
          {[{ label: 'Noleggi attivi', value: '3', color: '#00C896' }, { label: 'Ricavi/mese', value: '€ 179,70', color: '#a5b4fc' }].map(s => (
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 28, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        {TABS.map(t => (
          <button key={t.key} onClick={() => { setTab(t.key); setStep('catalogo'); }} style={{ padding: '10px 22px', background: 'none', border: 'none', cursor: 'pointer', color: tab === t.key ? '#00C896' : '#64748b', borderBottom: tab === t.key ? '2px solid #00C896' : '2px solid transparent', fontSize: 14, fontWeight: tab === t.key ? 600 : 400, marginBottom: -1 }}>{t.label}</button>
        ))}
      </div>
      {tab === 'noleggio' && (
        <>
          {step === 'catalogo' && <StepCatalogo onSelect={p => { setProduct(p); setStep('calcolatore'); }} />}
          {step === 'calcolatore' && product && <StepCalcolatore product={product} onBack={() => setStep('catalogo')} onProceed={cfg => { setConfig(cfg); setStep('attivazione'); }} />}
          {step === 'attivazione' && product && config && <StepAttivazione product={product} config={config} onBack={() => setStep('calcolatore')} onDone={() => { setStep('catalogo'); setProduct(null); setConfig(null); }} />}
        </>
      )}
      {tab === 'storico' && (
        <div>
          <div style={{ color: '#64748b', fontSize: 14, marginBottom: 16 }}>{storico.length} noleggi totali</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {storico.map(o => (
              <div key={o.id} style={{ background: 'rgba(15,23,42,0.8)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(0,200,150,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#00C896', fontWeight: 700, fontSize: 14, flexShrink: 0 }}>{o.cliente[0]}</div>
                <div style={{ flex: 1 }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontWeight: 700, color: '#00C896', fontSize: 16 }}>{fmt(o.rata)}<span style={{ fontSize: 11, color: '#64748b', fontWeight: 400 }}>/mese</span></div>
                ...
                <div style={{ fontSize: 11, color: '#475569', minWidth: 80 }}>{o.id}</div>
              </div>
            ))}
          </div>
        </div>
      )}
      {tab === 'impostazioni' && <PannelloImpostazioni onSave={() => toast('Impostazioni salvate!', 'success')} />}
    </div>
  );
}
