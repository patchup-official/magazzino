// App.jsx — Magazzino v3.0 con Auth integrata
// SOSTITUISCE frontend/src/App.jsx
...
function LoadingScreen() {
  return (
    <div style={{
      ...
      justifyContent: 'center', background: '#080e1f', color: '#475569',
      fontSize: 14, fontFamily: 'Inter, sans-serif', flexDirection: 'column', gap: 12,
    }}>
...
// ── Router principale — decide cosa mostrare in base al ruolo ─────────────
function AppRouter() {
  const { user, loading } = useAuth()
  if (loading)                       return <LoadingScreen />
  if (!user)                         return <Login />
  if (user.role === 'SUPER_ADMIN')   return <AdminDashboard />
  return <MainApp />  // ADMIN e OPERATOR
}

// ── App principale (ADMIN + OPERATOR) ────────────────────────────────────
function MainApp() {
  ...
  useEffect(() => {
    const f = (e) => { if (e.key === 'Escape') setShowCN(false) }
  ...
  const titles = {
    ...
    calendario: 'Calendario', storico: 'Storico acquisti', clienti: 'Clienti',
    importexport: 'Import / Export', protezione: 'Protezione Dispositivo',
    impostazioni: 'Impostazioni', valutazione: 'Valutazione Display',
    noleggio: 'Noleggio Subbyx', cassa: 'Chiusura Cassa',
  ...
  const pages = {
    ...
    calendario: Calendario, storico: Storico, clienti: Clienti,
    importexport: ImportExport, protezione: Protezione, impostazioni: Impostazioni,
    valutazione: ValutazioneDisplay, noleggio: Noleggio, cassa: Cassa,
    prenotazioni: PrenotazioneOrdini,
    operatori: GestioneOperatori,  // solo ADMIN
  }

  const PageComponent = pages[currentPage] || Dashboard

  // Nasconde "Gestione Operatori" agli OPERATOR
  const canSeeOperatori = user?.role === 'ADMIN' || user?.role === 'SUPER_ADMIN'
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#080e1f', overflow: 'hidden' }}>
      <Sidebar
        ...
        user={user}
        onLogout={logout}
        canSeeOperatori={canSeeOperatori}
      />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button
              ...
              style={{
                ...
                boxShadow: '0 2px 12px rgba(249,115,22,0.3)', letterSpacing: '.01em',
              }}
            >
...
// ── Modal "Crea nuovo" — invariato dal tuo originale ─────────────────────
function CNModal({ onClose, onSelect }) {
  const A = [
    ...
    { id: 'promemoria',      icon: '📌', label: 'Attività / promemoria',   desc: 'Crea promemoria',           c: '#f59e0b' },
  ]
  return (
    <div onClick={e => e.stopPropagation()} style={{
      ...
      <div style={{ padding: '22px 24px 14px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <div style={{ fontSize: 10, color: '#f97316', fontWeight: 700, letterSpacing: '.1em', marginBottom: 6, textTransform: 'uppercase' }}>ENOWN</div>
        ...
        <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, width: 32, height: 32, cursor: 'pointer', color: '#64748b', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>✕</button>
      </div>
