...
try {
db.run(`CREATE TABLE IF NOT EXISTS prenotazioni_ordini (
id TEXT PRIMARY KEY, cliente_id TEXT,
cliente_nome TEXT NOT NULL, cliente_telefono TEXT, cliente_email TEXT,
brand TEXT NOT NULL, modello TEXT NOT NULL, colore_variante TEXT,
tipo_riparazione TEXT, ricambio TEXT,
in_store INTEGER DEFAULT 1, note_dispositivo TEXT,
fornitore_id TEXT, fornitore_nome TEXT,
stato TEXT DEFAULT 'da_ordinare',
data_inserimento TEXT, data_ordine TEXT, data_arrivo TEXT,
caparra_attiva INTEGER DEFAULT 0, caparra_importo REAL,
caparra_totale REAL, caparra_metodo TEXT, caparra_note TEXT,
note_generali TEXT,
created_at TEXT DEFAULT (datetime('now')),
updated_at TEXT DEFAULT (datetime('now'))
)`);
db.run(`CREATE INDEX IF NOT EXISTS idx_prenord_stato ON prenotazioni_ordini(stato)`);
db.run(`CREATE INDEX IF NOT EXISTS idx_prenord_cliente ON prenotazioni_ordini(cliente_nome)`);
} catch(e) {}
...
prenotazioni_ordini: app.locals.get('SELECT COUNT(*) as n FROM prenotazioni_ordini')?.n || 0,
});
});
